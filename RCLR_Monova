...
function Library:SpawnNotify(Text, Time)
    ...
    Outer.Position = UDim2.new(0, 40, 0, Outer.Position.Y.Offset)
    TweenService:Create(Outer, TweenInfo.new(0.35, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
        Position = UDim2.new(0, 0, 0, Outer.Position.Y.Offset)
    }):Play()
    ...
    task.spawn(function()
        ...
        TweenService:Create(Outer, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
            Position = UDim2.new(0, 40, 0, Outer.Position.Y.Offset)
        }):Play()
...
function Library:CreateWindow(...)
    ...
    CornerCircle.MouseLeave:Connect(function() CornerCircle.BackgroundTransparency = 0.55; end);
    ...
    do
        ...
        InputService.InputBegan:Connect(function(Input)
            ...
            local Rad = InputService.TouchEnabled and 40 or 26;
...
function Library:SetGlass(Enabled, Transparency)
    ...
    if type(Transparency) == "number" then
        Library.GlassTransparency = math.clamp(Transparency, 0.3, 0.92)
    end
    Library.GlassTransparency = Library.GlassTransparency or 0.72

    local on = Library.GlassEnabled
    local t = Library.GlassTransparency
    local L_outer = on and t or 0
    local L_box = on and math.clamp(t - 0.02, 0.3, 0.9) or 0
    local L_main = on and math.clamp(t + 0.04, 0.35, 0.95) or 0
    ...
    pcall(function()
        if on then
            ...
            Library.DarkenAmount = math.clamp(Library.DarkenAmount or 8, 0, 30)
        end
        ...
        if Library.WindowInner then
            Library.WindowInner.BackgroundTransparency = L_outer - 0.02
        end
        if Library.DarkOverlay then
