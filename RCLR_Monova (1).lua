...
function Library:CreateWindow(...)
    ...
    pcall(function()
        ...
        Library.GlassTransparency = Library.GlassTransparency or 0.4
...
function Library:SetGlass(Enabled, Transparency)
    ...
    if type(Transparency) == "number" then
        Library.GlassTransparency = math.clamp(Transparency, 0, 0.7)
    end
    Library.GlassTransparency = Library.GlassTransparency or 0.4
    local on = Library.GlassEnabled == true
    local t = Library.GlassTransparency
    local L1 = on and math.clamp(t, 0.1, 0.55) or 0
    local L2 = on and math.clamp(t * 0.6, 0.05, 0.4) or 0
    pcall(function()
        if Library.WindowOuter then
            Library.WindowOuter.BackgroundColor3 = Library.BackgroundColor
            Library.WindowOuter.BackgroundTransparency = L1
        end
        if not Library.ScreenGui then return end
        for _, d in ipairs(Library.ScreenGui:GetDescendants()) do
            if d:IsA("Frame") then
                local nm = d.Name or ""
                if nm:find("Accent") or nm:find("Fill") or nm:find("Corner") or nm:find("Cursor") then
                else
...
function Library:ReapplyGlass()
    Library:SetGlass(Library.GlassEnabled ~= false, Library.GlassTransparency or 0.4)
end
