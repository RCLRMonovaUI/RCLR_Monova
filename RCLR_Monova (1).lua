...
function Library:CreateWindow(...)
    ...
    pcall(function() if Library.SetGlass then Library:SetGlass(true, Library.GlassTransparency or 0.9) end end)
...
function Library:SetGlass(Enabled, Transparency)
    ...
    if type(Transparency) == "number" then
        Library.GlassTransparency = math.clamp(Transparency, 0.5, 0.98)
    end
    Library.GlassTransparency = Library.GlassTransparency or 0.9
    ...
    -- REAL glass: outer almost fully clear so game world shows behind entire UI
    local L_outer = on and math.clamp(t, 0.82, 0.98) or 0
    local L_box   = on and math.clamp(t * 0.72, 0.45, 0.82) or 0
    local L_main  = on and math.clamp(t * 0.38, 0.2, 0.45) or 0
    Library.OuterGlassTransparency = L_outer

    pcall(function()
        if on then
            Library.UseDarken = true
            Library.DarkenAmount = 3
            if Library.IsMobile == true then
                Library.UseBlur = false
                Library.BlurSize = 0
            else
        ...
        for _, d in ipairs(Library.ScreenGui:GetDescendants()) do
            if d:IsA("Frame") or d:IsA("ScrollingFrame") then
                ...
                elseif nm == "TabBarOuter" or nm == "TabBarInner" then
                    ...
                    d.BackgroundTransparency = on and math.clamp(L_box * 0.5, 0.15, 0.4) or 0
        ...
        -- soft glass edge stroke on outer
        pcall(function()
...
function Library:ReapplyGlass()
    Library:SetGlass(Library.GlassEnabled ~= false, Library.GlassTransparency or 0.9)
end
