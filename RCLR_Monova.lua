...
function Library:UpdateBlur()
    ...
    if open then Library.BlurEffect.Enabled = true end
    ...
    if not open then Library.BlurEffect.Enabled = false end
end
...
function Library:AddBlurSlider(Groupbox)
    ...
    Groupbox:AddSlider('LinoriaBlurSize', {
        ...
        Callback = function(Value) Library:SetBlur(Value); end;
    });
end
...
function Library:SetKeybindTransparency(Value)
    ...
    if Library.KeybindFrame then
        ...
        if not Inner then Inner = Library.KeybindFrame:FindFirstChildOfClass('Frame'); end
    end
    if Inner then
...
function Library:AddKeybindTransparencySlider(Groupbox)
    Groupbox:AddSlider('LinoriaKeybindTransparency', {
        ...
        Default = 0;
        Min = 0; Max = 100; Rounding = 0; Suffix = '%';
        Callback = function(Value) Library:SetKeybindTransparency(Value / 100); end;
    });
end
...
function Library:SetFontSize(Size)
    ...
    for _, descendant in pairs(ScreenGui:GetDescendants()) do
        if descendant:IsA("TextLabel") or descendant:IsA("TextBox") or descendant:IsA("TextButton") then
            ...
            if offset then descendant.TextSize = Size + offset end
        end
    end
...
function Library:AddCorner(Inst, Radius)
    ...
    Library:Create('UICorner', { CornerRadius = UDim.new(0, r); Parent = Inst });
end
...
function Library:MakeDraggable(Instance, Cutoff, IsWindow)
    ...
    Instance.InputBegan:Connect(function(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
            if IsWindow and InputService.TouchEnabled and getgenv().RCLR_UILocked then
                return
            end
...
Library:GiveSignal(Heartbeat:Connect(function()
    ...
    for _, d in ipairs(Library.ScreenGui:GetDescendants()) do
        if d:IsA("ScrollingFrame") then
            ...
            if layout then
                local h = math.max(1, math.floor(layout.AbsoluteContentSize.Y + 20))
                if math.abs(d.CanvasSize.Y.Offset - h) > 1 then
                    d.CanvasSize = UDim2.fromOffset(0, h)
                end
            end
        end
    end
...
do
    ...
    function Funcs:AddColorPicker(Idx, Info)
        ...
        Library:AddCorner(PickerFrameOuter, 8)
        ...
        Library:AddCorner(SatVibMapOuter, 4)
        ...
        Library:AddCorner(SatVibMapInner, 3)
        ...
        Library:AddCorner(RgbBoxBase, 4)
        ...
        SatVibMap.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                ...
                EndedConn = InputService.InputEnded:Connect(function(EndInput)
                    if EndInput == Input or EndInput.UserInputType == Enum.UserInputType.Touch then
                        ChangedConn:Disconnect(); EndedConn:Disconnect(); Library:AttemptSave()
                    end
                end)
            end
        end);
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        local function BeginPicking()
            ...
            Event = InputService.InputBegan:Connect(function(Input)
                ...
                elseif Input.UserInputType == Enum.UserInputType.Touch then Key = 'Touch'; end;
        ...
        PickOuter.InputBegan:Connect(function(Input)
            if Library:MouseIsOverOpenedFrame() then return; end;
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then BeginPicking();
            elseif Input.UserInputType == Enum.UserInputType.MouseButton2 then OpenModeSelect();
            elseif Input.UserInputType == Enum.UserInputType.Touch then
                local StartPosition = Input.Position;
                local TouchMoved = false; local TouchEnded = false; local LongPressed = false;
                local ChangedConn; local EndedConn;
                ChangedConn = InputService.InputChanged:Connect(function(Change)
                    if Change == Input then
                        if (Change.Position - StartPosition).Magnitude > TouchMoveThreshold then TouchMoved = true; end;
                    end;
                end);
                EndedConn = InputService.InputEnded:Connect(function(EndInput)
                    if EndInput == Input then
                        ...
                        if (not LongPressed) and (not TouchMoved) then task.spawn(BeginPicking); end;
                    end;
                end);
...
do
        ...
        local function InitEvents(Button)
            local function WaitForEvent(event, timeout, validator)
                ...
                local connection = event:Once(function(...)
                    if type(validator) == 'function' and validator(...) then bindable:Fire(true) else bindable:Fire(false) end
                end)
    ...
    function Funcs:AddDivider()
        ...
        Library:AddCorner(DividerInner, 3)
    ...
    function Funcs:AddTextbox(Idx, Info)
        assert(Info.Text, 'AddTextbox: Missing Text.');
        local Textbox = {
            Value = Info.Default or '';
            Type = 'Textbox';
            Callback = Info.Callback or function(Value) end;
            Height = Info.Height or 60;
        };
        local Groupbox = self;
        local Container = Groupbox.Container;
        Library:CreateLabel({
        ...
        local BoxOuter = Library:Create('Frame', {
            BackgroundColor3 = Color3.new(0, 0, 0);
            BorderColor3 = Color3.new(0, 0, 0);
            Size = UDim2.new(1, -4, 0, Textbox.Height);
            ZIndex = 5;
            Parent = Container;
        });
        ...
        Box:GetPropertyChangedSignal('Text'):Connect(function() Textbox:SetValue(Box.Text) end);
        Box.FocusLost:Connect(function() Library:AttemptSave() end);
        function Textbox:OnChanged(Func)
            Textbox.Changed = Func;
            Func(Textbox.Value);
        end;
    ...
    function Funcs:AddSearchBox(Idx, Info)
        assert(Info.Text, 'AddSearchBox: Missing Text.');
        local Search = {
            Value = Info.Default or '';
            Type = 'SearchBox';
            Callback = Info.Callback or function(Value) end;
        ...
        local Icon = Library:Create('ImageLabel', {
            ...
            ImageRectOffset = Vector2.new(924, 964);
        ...
        function Search:SetValue(Value)
            Search.Value = Value or '';
            Box.Text = Search.Value;
            Library:SafeCallback(Search.Callback, Search.Value);
            Library:SafeCallback(Search.Changed, Search.Value);
        end;
        Box:GetPropertyChangedSignal('Text'):Connect(function() Search:SetValue(Box.Text) end);
        Box.FocusLost:Connect(function() Library:AttemptSave() end);
        function Search:OnChanged(Func)
            Search.Changed = Func;
            Func(Search.Value);
        end;
        Groupbox:Resize();
    ...
    function Funcs:AddSegment(Idx, Info)
        assert(Info.Values, 'AddSegment: Missing Values.');
        local Segment = {
        ...
        local Buttons = {};
        local SegCount = #Info.Values;
        for i, V in ipairs(Info.Values) do
            local Seg = Library:Create('Frame', {
                BackgroundColor3 = (Segment.Value == V) and Library.AccentColor or Library.MainColor;
                BorderColor3 = Color3.new(0, 0, 0);
                BorderMode = Enum.BorderMode.Inset;
                Size = UDim2.new(1 / SegCount, 0, 1, 0);
                ZIndex = 7;
                Parent = Inner;
            });
            Library:AddCorner(Seg, 5)
            Library:AddToRegistry(Seg, { BackgroundColor3 = (Segment.Value == V) and 'AccentColor' or 'MainColor'; BorderColor3 = 'Black'; });
            ...
            Buttons[i] = {}
            Buttons[i].Set = function(Selected)
                Seg.BackgroundColor3 = Selected and Library.AccentColor or Library.MainColor;
                Library.RegistryMap[Seg].Properties.BackgroundColor3 = Selected and 'AccentColor' or 'MainColor';
            end
            Seg.InputBegan:Connect(function(Input)
                if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and not Library:MouseIsOverOpenedFrame() then
                    for _, B in ipairs(Buttons) do B.Set(false) end
                    Buttons[i].Set(true);
                    Segment.Value = V;
                    Library:SafeCallback(Segment.Callback, Segment.Value);
                    Library:AttemptSave();
                end
            end);
        end
        function Segment:SetValue(V)
            for i, Val in ipairs(Info.Values) do Buttons[i].Set(Val == V) end
            Segment.Value = V;
            Library:SafeCallback(Segment.Callback, Segment.Value);
        end
        Groupbox:Resize();
    ...
    function Funcs:AddProgress(Idx, Info)
        assert(Info.Text, 'AddProgress: Missing Text.');
        local Progress = {
        ...
        Library:AddCorner(BarFill, 4)
        Library:AddToRegistry(BarFill, { BackgroundColor3 = 'AccentColor'; BorderColor3 = 'AccentColorDark'; });
        local BarLabel = Library:CreateLabel({
            Size = UDim2.new(1, 0, 1, 0);
            TextSize = Library.FontSize;
            Text = '0%';
            ZIndex = 8;
        ...
        function Progress:Display()
            local pct = (Progress.Value - Progress.Min) / (Progress.Max - Progress.Min);
            pct = math.clamp(pct, 0, 1);
            local W = math.floor(BarInner.AbsoluteSize.X * pct + 0.5);
            TweenService:Create(BarFill, TweenInfo.new(0.18, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                Size = UDim2.new(0, W, 1, 0);
            }):Play();
            BarLabel.Text = string.format('%d/%d (%d%%)', Progress.Value, Progress.Max, math.floor(pct * 100 + 0.5));
        end;
        function Progress:SetValue(Value)
            Progress.Value = math.clamp(Value, Progress.Min, Progress.Max);
            Progress:Display();
            Library:SafeCallback(Progress.Callback, Progress.Value);
        end;
        BarInner:GetPropertyChangedSignal('AbsoluteSize'):Connect(function() Progress:Display() end);
        Progress:Display();
        Groupbox:Resize();
        Options[Idx] = Progress;
        return Progress;
    end;

    function Funcs:AddRangeSlider(Idx, Info)
        assert(Info.Text, 'AddRangeSlider: Missing Text.');
        local Slider = {
            MinValue = Info.DefaultMin or Info.Min or 0;
            MaxValue = Info.DefaultMax or Info.Max or 100;
            Min = Info.Min or 0;
            Max = Info.Max or 100;
            Rounding = Info.Rounding or 0;
            MaxSize = 232;
            Type = 'RangeSlider';
            Callback = Info.Callback or function(MinV, MaxV) end;
        };
        local Groupbox = self;
        local Container = Groupbox.Container;
        Library:CreateLabel({
        ...
        local HandleL = Library:Create('Frame', {
            AnchorPoint = Vector2.new(0.5, 0.5);
            BackgroundColor3 = Library.AccentColor;
            BorderColor3 = Library.AccentColorDark;
        ...
        function Slider:Display()
            ...
            Slider.MaxSize = math.max(1, math.floor(W + 0.5));
            local PL = (Slider.MinValue - Slider.Min) / (Slider.Max - Slider.Min);
            local PR = (Slider.MaxValue - Slider.Min) / (Slider.Max - Slider.Min);
            local XL = math.clamp(math.floor(PL * Slider.MaxSize + 0.5), 0, Slider.MaxSize);
            local XR = math.clamp(math.floor(PR * Slider.MaxSize + 0.5), 0, Slider.MaxSize);
            Fill.Position = UDim2.new(0, XL, 0, 0);
            Fill.Size = UDim2.new(0, math.max(1, XR - XL), 1, 0);
            HandleL.Position = UDim2.new(0, XL, 0.5, 0);
            HandleR.Position = UDim2.new(0, XR, 0.5, 0);
            Label.Text = string.format('%s - %s', Slider.MinValue, Slider.MaxValue);
        end;
        function Slider:SetValue(MinV, MaxV)
            Slider.MinValue = math.clamp(MinV, Slider.Min, Slider.Max);
            Slider.MaxValue = math.clamp(MaxV, Slider.MinValue, Slider.Max);
            Slider:Display();
            Library:SafeCallback(Slider.Callback, Slider.MinValue, Slider.MaxValue);
        end;
        local function ValueFromX(X)
            local P = math.clamp(X / Slider.MaxSize, 0, 1);
            return Round(Slider.Min + P * (Slider.Max - Slider.Min));
        end
        local function BeginDrag(Input, Mode)
            local function Update(PosX)
                local RelX = PosX - Inner.AbsolutePosition.X;
                local V = ValueFromX(RelX);
                if Mode == 'L' then Slider.MinValue = math.min(V, Slider.MaxValue);
                elseif Mode == 'R' then Slider.MaxValue = math.max(V, Slider.MinValue);
                else
                    Slider.MinValue = math.min(V, Slider.MaxValue);
                    Slider.MaxValue = math.max(V, Slider.MinValue);
                end
                Slider:Display();
                Library:SafeCallback(Slider.Callback, Slider.MinValue, Slider.MaxValue);
            end
            Update(Input.Position.X);
            local CC = InputService.InputChanged:Connect(function(C)
                if C.UserInputType == Enum.UserInputType.MouseMovement or C == Input then
                    Update(C.Position.X);
                end
            end);
            local EC;
            EC = InputService.InputEnded:Connect(function(E)
        ...
        HandleL.InputBegan:Connect(function(I)
            if I.UserInputType == Enum.UserInputType.MouseButton1 or I.UserInputType == Enum.UserInputType.Touch then BeginDrag(I, 'L') end
        end);
        HandleR.InputBegan:Connect(function(I)
            if I.UserInputType == Enum.UserInputType.MouseButton1 or I.UserInputType == Enum.UserInputType.Touch then BeginDrag(I, 'R') end
        end);
        Inner:GetPropertyChangedSignal('AbsoluteSize'):Connect(function() Slider:Display() end);
        Slider:Display();
        Groupbox:Resize();
        Options[Idx] = Slider;
        return Slider;
    ...
    function Funcs:AddConfirm(Idx, Info)
        assert(Info.Text, 'AddConfirm: Missing Text.');
        assert(Info.Func, 'AddConfirm: Missing Func.');
        local Confirm = {
            Type = 'Confirm';
            Callback = Info.Func;
        };
        ...
        Library:CreateLabel({
            ...
            TextSize = Library.FontSize;
            Text = Info.Body or 'This action cannot be undone.';
            TextWrapped = true;
            TextXAlignment = Enum.TextXAlignment.Left;
            TextYAlignment = Enum.TextYAlignment.Top;
            ZIndex = 502;
        ...
        local YesBtn = Library:Create('Frame', {
            BackgroundColor3 = Library.MainColor;
        ...
        local NoBtn = Library:Create('Frame', {
            BackgroundColor3 = Library.MainColor;
            BorderColor3 = Library.OutlineColor;
            BorderMode = Enum.BorderMode.Inset;
        ...
        YesBtn.InputBegan:Connect(function(I)
            if I.UserInputType == Enum.UserInputType.MouseButton1 or I.UserInputType == Enum.UserInputType.Touch then
                Popup.Visible = false;
                Library.OpenedFrames[Popup] = nil;
                Library:SafeCallback(Confirm.Callback);
            end
        end);
        NoBtn.InputBegan:Connect(function(I)
            if I.UserInputType == Enum.UserInputType.MouseButton1 or I.UserInputType == Enum.UserInputType.Touch then
                Popup.Visible = false;
                Library.OpenedFrames[Popup] = nil;
            end
        ...
        Outer.InputBegan:Connect(function(I)
            if I.UserInputType == Enum.UserInputType.MouseButton1 or I.UserInputType == Enum.UserInputType.Touch then
                Popup.Visible = true;
                Library.OpenedFrames[Popup] = true;
            end
        end);
        Confirm.Popup = Popup;
        Groupbox:Resize();
    ...
    function Funcs:AddBind(Idx, Info)
        assert(Info.Text, 'AddBind: Missing Text.');
        local Bind = {
        ...
        local BindOuter = Library:Create('Frame', {
            BackgroundColor3 = Color3.new(0, 0, 0);
            BorderColor3 = Color3.new(0, 0, 0);
            Position = UDim2.new(1, -76, 0, 0);
            Size = UDim2.new(0, 76, 0, 15);
            ZIndex = 6;
            Parent = Container;
        });
        Library:AddCorner(BindOuter, 3)
        ...
        Library:AddCorner(BindInner, 3)
        Library:AddToRegistry(BindInner, { BackgroundColor3 = 'BackgroundColor'; BorderColor3 = 'OutlineColor'; });
        ...
        local function DisplayValue()
            local Parts = {};
            if Bind.Modifiers.Ctrl then table.insert(Parts, 'Ctrl') end
            if Bind.Modifiers.Shift then table.insert(Parts, 'Shift') end
            if Bind.Modifiers.Alt then table.insert(Parts, 'Alt') end
            table.insert(Parts, Bind.Value);
            BindLabel.Text = table.concat(Parts, '+');
        end
        BindOuter.InputBegan:Connect(function(I)
            if Library:MouseIsOverOpenedFrame() then return end
            if I.UserInputType == Enum.UserInputType.MouseButton1 or I.UserInputType == Enum.UserInputType.Touch then
                if Picking then return end
                Picking = true;
                BindLabel.Text = '...';
                local Event;
                Event = InputService.InputBegan:Connect(function(In)
                    if In.UserInputType == Enum.UserInputType.Keyboard then
                        Bind.Modifiers.Ctrl = InputService:IsKeyDown(Enum.KeyCode.LeftControl) or InputService:IsKeyDown(Enum.KeyCode.RightControl);
                        Bind.Modifiers.Shift = InputService:IsKeyDown(Enum.KeyCode.LeftShift) or InputService:IsKeyDown(Enum.KeyCode.RightShift);
                        Bind.Modifiers.Alt = InputService:IsKeyDown(Enum.KeyCode.LeftAlt) or InputService:IsKeyDown(Enum.KeyCode.RightAlt);
                        Bind.Value = In.KeyCode.Name;
                        Picking = false;
                        DisplayValue();
                        Library:SafeCallback(Bind.Callback, Bind.Value);
                        Library:AttemptSave();
                        Event:Disconnect();
                    end
                end);
    ...
    function Funcs:AddChart(Idx, Info)
        ...
        local ChartOuter = Library:Create('Frame', {
            BackgroundColor3 = Color3.new(0, 0, 0);
            BorderColor3 = Color3.new(0, 0, 0);
            Size = UDim2.new(1, -4, 0, Info.Height or 80);
            ZIndex = 5;
            Parent = Container;
        });
        Library:AddCorner(ChartOuter, 5)
        local ChartInner = Library:Create('Frame', {
            BackgroundColor3 = Library.BackgroundColor;
            BorderColor3 = Library.OutlineColor;
            BorderMode = Enum.BorderMode.Inset;
            ClipsDescendants = true;
        ...
        Library:AddCorner(ChartInner, 5)
        Library:AddToRegistry(ChartInner, { BackgroundColor3 = 'BackgroundColor'; BorderColor3 = 'OutlineColor'; });
        local MaxPoints = Info.Points or 60;
        local PointFrames = {};
        local function Redraw()
            for _, P in ipairs(PointFrames) do
                if P.Frame then P.Frame:Destroy() end
            end
            PointFrames = {};
            local N = #Chart.Values;
            if N < 2 then return end
            local W = ChartInner.AbsoluteSize.X;
            local H = ChartInner.AbsoluteSize.Y;
            for i = 1, N - 1 do
                local V1 = Chart.Values[i];
                local V2 = Chart.Values[i + 1];
                local X1 = ((i - 1) / (MaxPoints - 1)) * W;
                local X2 = (i / (MaxPoints - 1)) * W;
                local Y1 = H - (V1 / Chart.Max) * H;
                local Y2 = H - (V2 / Chart.Max) * H;
                local Dx = X2 - X1;
                local Dy = Y2 - Y1;
                ...
                table.insert(PointFrames, { Frame = Seg });
            end
        end
        function Chart:Push(Value)
            table.insert(Chart.Values, math.clamp(Value, 0, Chart.Max));
            while #Chart.Values > MaxPoints do table.remove(Chart.Values, 1) end
            Redraw();
        end;
        ...
        ChartInner:GetPropertyChangedSignal('AbsoluteSize'):Connect(Redraw);
        Groupbox:Resize();
        Options[Idx] = Chart;
        return Chart;
    end;
    ...
    function Funcs:AddTable(Idx, Info)
        assert(Info.Columns, 'AddTable: Missing Columns.');
        local Table = {
        ...
        for _, Col in ipairs(Table.Columns) do
            local Cell = Library:Create('Frame', {
                BackgroundTransparency = 1;
                Size = UDim2.new(1 / #Table.Columns, 0, 1, 0);
                ZIndex = 6;
                Parent = Header;
            });
            Library:CreateLabel({
        ...
        function Table:Render()
            ...
            for rIdx, Row in ipairs(Table.Rows) do
                local RowFrame = Library:Create('Frame', {
                    BackgroundColor3 = (rIdx % 2 == 0) and Library.BackgroundColor or Library.MainColor;
                    BorderColor3 = Library.OutlineColor;
                    BorderMode = Enum.BorderMode.Inset;
                    Size = UDim2.new(1, 0, 0, 18);
                    ZIndex = 5;
                ...
                Library:AddCorner(RowFrame, 3)
                Library:AddToRegistry(RowFrame, { BackgroundColor3 = (rIdx % 2 == 0) and 'BackgroundColor' or 'MainColor'; BorderColor3 = 'OutlineColor'; });
                Library:Create('UIListLayout', {
                    FillDirection = Enum.FillDirection.Horizontal;
                    SortOrder = Enum.SortOrder.LayoutOrder;
                    Parent = RowFrame;
    ...
    function Funcs:AddImage(Idx, Info)
        assert(Info.Image, 'AddImage: Missing Image.');
        local Image = {
            Image = Info.Image;
            Type = 'Image';
        ...
        local ImageOuter = Library:Create('Frame', {
            BackgroundColor3 = Color3.new(0, 0, 0);
            BorderColor3 = Color3.new(0, 0, 0);
            Size = UDim2.new(1, -4, 0, Info.Height or 90);
            ZIndex = 5;
            Parent = Container;
        });
        Library:AddCorner(ImageOuter, 6)
        ...
        Library:AddCorner(ImageLabel, 6)
        Library:AddToRegistry(ImageLabel, { BackgroundColor3 = 'BackgroundColor' });
        Library:OnHighlight(ImageOuter, ImageOuter, { BorderColor3 = 'AccentColor' }, { BorderColor3 = 'Black' });
        ImageOuter.InputBegan:Connect(function(I)
            if I.UserInputType == Enum.UserInputType.MouseButton1 or I.UserInputType == Enum.UserInputType.Touch then
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        function Slider:Display()
            ...
            TweenService:Create(Fill, TweenInfo.new(0.22, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
        ...
        SliderOuter.MouseEnter:Connect(function()
            TweenService:Create(Glow, TweenInfo.new(0.15), { BackgroundTransparency = 0.85 }):Play()
        end)
...
do
    ...
    function Library:ConfigureNotifications(Cfg)
        ...
        if C.ClipDescendants then
            if not SizeConstraint then SizeConstraint = Library:Create('UISizeConstraint', { Parent = Library.NotificationArea }); end;
            SizeConstraint.MaxSize = Vector2.new(math.huge, C.MaxHeight);
        elseif SizeConstraint then SizeConstraint:Destroy(); end;
    ...
    Library:AddCorner(WatermarkOuter, 5)
    ...
    Library:AddCorner(InnerFrame, 4)
    ...
    Library:AddCorner(KeybindOuter, 5)
...
function Library:RefreshKeybinds()
    for _, kp in ipairs(Library.KeyPickerList) do
        if not kp.NoUI then pcall(function() kp:Update() end) end
    end
end
...
function Library:SpawnNotify(Text, Time)
    ...
    local GradientFrame = Library:Create('Frame', { BackgroundColor3 = Library.MainColor; BackgroundTransparency = 1; BorderSizePixel = 0; Position = UDim2.new(0, 1, 0, 1); Size = UDim2.new(1, -2, 1, -2); ZIndex = 102; Parent = Inner });
    Library:AddCorner(GradientFrame, 5)
    ...
    local InnerLabel = Library:CreateLabel({ Position = UDim2.new(0, 8, 0, 0); Size = UDim2.new(1, -8, 1, 0); Text = Text; TextXAlignment = Enum.TextXAlignment.Left; TextSize = 13; ZIndex = 103; Parent = GradientFrame });
    InnerLabel.TextTransparency = 1
    local BarSide = Library.NotifyConfig.BarSide or "Bottom";
    local AccentBarPos, AccentBarSize;
    if BarSide == "Top" then AccentBarPos = UDim2.new(0, -1, 0, -1); AccentBarSize = UDim2.new(1, 2, 0, 3);
    elseif BarSide == "Bottom" then AccentBarPos = UDim2.new(0, -1, 1, -2); AccentBarSize = UDim2.new(1, 2, 0, 3);
    elseif BarSide == "Left" then AccentBarPos = UDim2.new(0, -1, 0, -1); AccentBarSize = UDim2.new(0, 3, 1, 2);
    else AccentBarPos = UDim2.new(1, -2, 0, -1); AccentBarSize = UDim2.new(0, 3, 1, 2); end;
    ...
    TweenService:Create(Outer, TweenInfo.new(0.35, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
    ...
    TweenService:Create(InnerStroke, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
        Transparency = NotifyTransparency
    }):Play()
    TweenService:Create(GradientFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
        BackgroundTransparency = NotifyTransparency
    }):Play()
    TweenService:Create(InnerLabel, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
        TextTransparency = 0
    }):Play()
    ...
    task.spawn(function()
        task.wait(Time or 5);
        TweenService:Create(Outer, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
            Size = UDim2.fromOffset(0, H)
        }):Play()
        TweenService:Create(Inner, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
            BackgroundTransparency = 1
        }):Play()
        TweenService:Create(InnerStroke, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
            Transparency = 1
        }):Play()
        TweenService:Create(GradientFrame, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
            BackgroundTransparency = 1
        }):Play()
        TweenService:Create(InnerLabel, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
            TextTransparency = 1
        }):Play()
        TweenService:Create(AccentBar, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
            BackgroundTransparency = 1
        }):Play()
        task.wait(0.4);
        Outer:Destroy();
...
function Library:CreateWindow(...)
    ...
    task.spawn(function()
        ...
        if success and info and info.Name then MapNameLabel.Text = info.Name;
        else MapNameLabel.Text = game.Name or "Unknown Map"; end
    end)
    ...
    Library:AddCorner(TabBarInner, 5)
    ...
    Library:AddCorner(MainSectionOuter, 5)
    ...
    Library:Create('UICorner', { CornerRadius = UDim.new(1, 0); Parent = CornerCircle; });
    CornerCircle.Active = true;
    CornerCircle.MouseEnter:Connect(function() CornerCircle.BackgroundTransparency = 0.2; end);
    CornerCircle.MouseLeave:Connect(function() CornerCircle.BackgroundTransparency = 0.7; end);

    do
        local MinW = 420; local MinH = 340;
            ...
            ResizeConn = InputService.InputChanged:Connect(function(Change)
                ...
                if Library.WireframeDrag then
                    if not HasMoved then
                        ...
                        Library:AddCorner(Wireframe, 10)
                        Library:Create('UIStroke', {
                            Color = Library.AccentColor;
                            Thickness = 1;
                            ApplyStrokeMode = Enum.ApplyStrokeMode.Border;
        ...
        local function OnHandlePressed(Position, InputType) StartResize(Position, InputType); end;
        ...
        InputService.InputBegan:Connect(function(Input)
            ...
            local Rad = InputService.TouchEnabled and 40 or 24;
            local DX = CenterX - P.X; local DY = CenterY - P.Y;
            if (DX * DX) + (DY * DY) <= (Rad * Rad) then OnHandlePressed(P, Input.UserInputType); end
        end)
    end
    ...
    function Window:AddTab(Name)
        ...
        for _, Side in next, { LeftSide, RightSide } do
            Side:WaitForChild('UIListLayout'):GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
                ...
                if layout then
                    Side.CanvasSize = UDim2.fromOffset(0, layout.AbsoluteContentSize.Y + 20)
                end
            end)
        end
    ...
    function Library:Toggle()
        ...
        if opening then
            ...
            Outer.Size = UDim2.fromOffset(
                math.max(50, fullSize.X.Offset * 0.96),
                math.max(50, fullSize.Y.Offset * 0.96)
            );
            Outer.BackgroundTransparency = 0.25;
            TweenService:Create(Outer, TweenInfo.new(0.18, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
                Size = fullSize;
                BackgroundTransparency = 0;
            }):Play();
        else
            local t = TweenService:Create(Outer, TweenInfo.new(0.16, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
                Size = UDim2.fromOffset(
                    math.max(40, fullSize.X.Offset * 0.94),
                    math.max(40, fullSize.Y.Offset * 0.94)
                );
...
if InputService.TouchEnabled then
    ...
    local function Bind(Btn, Outer, ClickAction, IsLockButton)
        ...
        InputService.InputChanged:Connect(function(input)
            if input == dragInput and dragging then
                ...
                if hasMoved and (IsUnlocked or not IsLockButton) then
    ...
    Bind(ToggleBtn, ToggleOuter, function() Library:Toggle() end, false)
    Bind(LockBtn, LockOuter, function()
        IsUnlocked = not IsUnlocked
        getgenv().RCLR_UILocked = not IsUnlocked
        ...
        Library:Notify(IsUnlocked and "UI unlocked - dragging enabled" or "UI locked - dragging disabled", 2)
    end, true)
end
...
function Library:ApplyBlackWhiteTheme()
    ...
    pcall(function() Library:ReapplyGlass() end)
end
...
function Library:SetGlass(Enabled, Transparency)
    ...
    if type(Transparency) == "number" then
        Library.GlassTransparency = math.clamp(Transparency, 0.05, 0.85)
    end
    Library.GlassTransparency = Library.GlassTransparency or 0.15

    local on = Library.GlassEnabled
    local t = Library.GlassTransparency
    local L_outer = on and t or 0
    local L_box = on and math.clamp(t - 0.02, 0.05, 0.85) or 0
    local L_main = on and math.clamp(t + 0.04, 0.05, 0.9) or 0
    ...
    pcall(function()
        if on then
            ...
            Library.BlurSize = math.clamp(Library.BlurSize or 14, 8, 24)
            Library.UseDarken = true
            Library.DarkenAmount = math.clamp(Library.DarkenAmount or 60, 20, 80)
        end
        ...
        if Library.WindowInner then
            Library.WindowInner.BackgroundTransparency = math.clamp(L_outer - 0.02, 0, 0.9)
        end
        if Library.DarkOverlay then
        ...
        for _, d in ipairs(Library.ScreenGui:GetDescendants()) do
            if d:IsA("Frame") or d:IsA("ScrollingFrame") then
                ...
                if nm:find("Fill") or nm:find("Accent") or nm:find("Corner")
                    or nm:find("Cursor") or nm:find("Indicator") or nm:find("Highlight")
                    or nm:find("Blocker") then
                elseif d == Library.WindowOuter or nm == "Outer" then
...
function Library:CreateLogo(Info)
    ...
    local size = tonumber(Info.TextSize) or 64
    local speed = tonumber(Info.SpinSpeed) or 40
    ...
    holder.Size = UDim2.fromOffset(size * 3, size * 3)
    holder.Parent = gui
    ...
    local t = 0
    local swingRange = 60
    local swingSpeed = speed / 40
    local conn = RunService.RenderStepped:Connect(function(dt)
        if Library.Toggled ~= true then gui.Enabled = false return end
        gui.Enabled = true
        t = t + dt * swingSpeed
        local swing = math.sin(t * math.pi / 2.4)
        holder.Rotation = swing * swingRange
        label.TextColor3 = Library.AccentColor
    end)
    Library._LogoSpinConn = conn
    table.insert(Library.Signals, conn)
    Library._LogoGui = gui
    Library._LogoSync = function() gui.Enabled = Library.Toggled == true end
end
