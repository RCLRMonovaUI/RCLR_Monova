...
function Library:CreateWindow(...)
    ...
    local TabListLayout = Library:Create('UIListLayout', {
        Padding = UDim.new(0, Config.TabPadding or 1);
        FillDirection = Enum.FillDirection.Horizontal;
        SortOrder = Enum.SortOrder.LayoutOrder;
    ...
    local function RefreshTabCanvas()
        pcall(function()
            task.defer(function()
                local w = math.max(TabListLayout.AbsoluteContentSize.X + 4, 4)
    ...
    local ResizeHover = Color3.fromRGB(120, 120, 120);
    CornerCircle.MouseEnter:Connect(function()
        CornerCircle.BackgroundColor3 = ResizeHover;
        CornerCircle.BackgroundTransparency = 0;
        CornerCircle.Size = UDim2.fromOffset(16, 16);
    end);
    CornerCircle.MouseLeave:Connect(function()
        CornerCircle.BackgroundColor3 = Library.AccentColor or ResizeDefault;
        if CornerCircle.BackgroundColor3.R > 0.85 then
            CornerCircle.BackgroundColor3 = ResizeDefault;
        end
        CornerCircle.BackgroundTransparency = 0.2;
        CornerCircle.Size = UDim2.fromOffset(14, 14);
    end);
...
function Library:CreateLiveStudUI(Info)
    ...
    local Rate = tonumber(Info.UpdateRate) or 0.1
    local W, H = 145, 86
    ...
    Root.Position = Side == "Right" and UDim2.new(1, -(W + 12), 0, 72) or UDim2.new(0, 12, 0, 72)
    Root.ZIndex = 90
    Root.Parent = Gui
    ...
    title.Size = UDim2.new(1, -14, 0, 14)
    ...
    local function row(y, letter)
        local L = Instance.new("TextLabel")
        L.BackgroundTransparency = 1
        L.Position = UDim2.fromOffset(10, y)
        L.Size = UDim2.fromOffset(12, 14)
        ...
        V.Position = UDim2.new(1, -72, 0, y)
        V.Size = UDim2.fromOffset(64, 14)
    ...
    local xV, xB, xL = row(24, "X")
    local yV, yB, yL = row(44, "Y")
    local zV, zB, zL = row(64, "Z")
    pcall(function() Library:MakeDraggable(Root, 20, false) end)
    ...
    local conn = RunService.Heartbeat:Connect(function(dt)
        ...
        local bg = Library.BackgroundColor or Color3.fromRGB(8, 8, 8)
        Root.BackgroundColor3 = bg
        Root.BackgroundTransparency = Library.GlassEnabled and 0.22 or 0.1
        edge.BackgroundColor3 = ac
        title.TextColor3 = fc
        xB.BackgroundColor3, yB.BackgroundColor3, zB.BackgroundColor3 = ac, ac, ac
        xV.TextColor3, yV.TextColor3, zV.TextColor3 = fc, fc, fc
        xL.TextColor3, yL.TextColor3, zL.TextColor3 = fc, fc, fc
    ...
    function API:SetVisible(v) Root.Visible = not not v; Gui.Enabled = not not v end
