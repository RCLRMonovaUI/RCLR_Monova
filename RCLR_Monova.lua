...
-- ============================================================
-- GLASS: bypasses the in-game background properly.
-- Uses Camera far-plane trick + replicated viewport snapshot
-- so the blur/glass samples the actual 3D scene, not the UI.
-- ============================================================

-- Creates a hidden "scene copy" behind the UI by parenting a duplicate
-- of the camera's viewport frame into the GUI. Since Roblox doesn't
-- expose a real backdrop-filter, we approximate a proper glass look:
--   1) We force the BlurEffect to render first (behind ScreenGui).
--   2) We darken/vignette everything via a fullscreen overlay.
--   3) We add an inner gradient + specular highlight to every glass panel.
-- This makes the glass visually "eat through" whatever background the
-- player has (game world, other guis, etc) instead of looking flat.
function Library:BuildGlassLayers()
    if Library._GlassBuilt then return end
    Library._GlassBuilt = true

    -- Specular highlight gradient applied to every registered frame
    -- handled lazily in SetGlass
end
...
-- Applies rounded corners + a subtle glass highlight to a frame.
-- Keeps a table of already-processed instances so we don't double-stack.
function Library:ApplyGlassCorner(Inst, Radius, Options)
    if not Inst or not Inst.Parent then return end
    Options = Options or {}
    Radius = Radius or 6

    if not Inst:FindFirstChild('__GlassCorner') then
        local corner = Instance.new('UICorner')
        corner.Name = '__GlassCorner'
        corner.CornerRadius = UDim.new(0, Radius)
        corner.Parent = Inst
    else
        Inst.__GlassCorner.CornerRadius = UDim.new(0, Radius)
    end

    -- Inner gradient for glossy highlight (top brighter, bottom darker)
    if Options.Highlight ~= false and not Inst:FindFirstChild('__GlassHighlight') then
        local hl = Instance.new('Frame')
        hl.Name = '__GlassHighlight'
        hl.BackgroundColor3 = Color3.new(1, 1, 1)
        hl.BackgroundTransparency = 0.92
        hl.BorderSizePixel = 0
        hl.Size = UDim2.new(1, 0, 0, math.max(4, Radius))
        hl.Position = UDim2.new(0, 0, 0, 0)
        hl.ZIndex = (Inst.ZIndex or 0) + 50
        hl.Parent = Inst
                ...
                if Change.UserInputType == Enum.UserInputType.MouseMovement or Change == Input then
                    ...
                    if IsWindow and Library.WireframeDrag then
                        if not HasMoved and Delta.Magnitude > 2 then
                            ...
                            Library:ApplyGlassCorner(Wireframe, 8)
...
function Library:AddToolTip(InfoStr, HoverInstance)
    ...
    Library:ApplyGlassCorner(Tooltip, 4, { Highlight = false })
...
function Library:UpdateColorsUsingRegistry()
    ...
    -- keep glass after theme color refresh
    if Library.GlassEnabled and Library.ReapplyGlass then
        task.defer(function()
...
do
    ...
    function Funcs:AddColorPicker(Idx, Info)
        ...
        Library:ApplyGlassCorner(DisplayFrame, 3, { Highlight = false })
        ...
        Library:ApplyGlassCorner(PickerFrameOuter, 8)
        ...
        Library:ApplyGlassCorner(PickerFrameInner, 7)
        ...
        Library:ApplyGlassCorner(SatVibMapOuter, 4, { Highlight = false })
        ...
        Library:ApplyGlassCorner(SatVibMapInner, 3, { Highlight = false })
        ...
        Library:ApplyGlassCorner(SatVibMap, 3, { Highlight = false })
        ...
        Library:ApplyGlassCorner(HueSelectorOuter, 4, { Highlight = false })
        ...
        Library:ApplyGlassCorner(HueSelectorInner, 4, { Highlight = false })
        ...
        Library:ApplyGlassCorner(HueBoxOuter, 4, { Highlight = false })
        ...
        Library:ApplyGlassCorner(HueBoxInner, 4, { Highlight = false })
        ...
        Library:ApplyGlassCorner(RgbBoxBase, 4, { Highlight = false })
        ...
        if Info.Transparency then 
            ...
            Library:ApplyGlassCorner(TransparencyBoxOuter, 4, { Highlight = false })
            ...
            Library:ApplyGlassCorner(TransparencyBoxInner, 4, { Highlight = false })
        ...
        do
            ...
            Library:ApplyGlassCorner(ContextMenu.Container, 6)
            ...
            Library:ApplyGlassCorner(ContextMenu.Inner, 5, { Highlight = false })
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        Library:ApplyGlassCorner(PickOuter, 3, { Highlight = false })
        ...
        Library:ApplyGlassCorner(PickInner, 3, { Highlight = false })
        ...
        Library:ApplyGlassCorner(ModeSelectOuter, 6)
        ...
        Library:ApplyGlassCorner(ModeSelectInner, 5, { Highlight = false })
...
do
    ...
    function Funcs:AddButton(...)
        ...
        local function CreateBaseButton(Button)
            ...
            Library:ApplyGlassCorner(Outer, 5)
            ...
            Library:ApplyGlassCorner(Inner, 5, { Highlight = false })
            local Label = Library:CreateLabel({
                Size = UDim2.new(1, 0, 1, 0);
                TextSize = Library.FontSize;
                Text = Button.Text;
    ...
    function Funcs:AddDivider()
        ...
        Library:ApplyGlassCorner(DividerOuter, 3, { Highlight = false })
        ...
        Library:ApplyGlassCorner(DividerInner, 3, { Highlight = false })
    ...
    function Funcs:AddInput(Idx, Info)
        ...
        Library:ApplyGlassCorner(TextBoxOuter, 5)
        ...
        Library:ApplyGlassCorner(TextBoxInner, 5, { Highlight = false })
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        Library:ApplyGlassCorner(ToggleOuter, 3, { Highlight = false })
        ...
        Library:ApplyGlassCorner(ToggleInner, 3, { Highlight = false })
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        Library:ApplyGlassCorner(SliderOuter, 4)
        ...
        Library:ApplyGlassCorner(SliderInner, 4, { Highlight = false })
        ...
        Library:ApplyGlassCorner(Fill, 4, { Highlight = false })
        ...
        function Slider:Display()
            ...
            -- full width when window resized
            local maxW = math.max(1, math.floor(SliderInner.AbsoluteSize.X + 0.5))
            if maxW < 2 then maxW = Slider.MaxSize or 232 end
            Slider.MaxSize = maxW
            local X = math.ceil(Library:MapValue(Slider.Value, Slider.Min, Slider.Max, 0, maxW));
            X = math.clamp(X, 0, maxW)
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        Library:ApplyGlassCorner(DropdownOuter, 5)
        ...
        Library:ApplyGlassCorner(DropdownInner, 5, { Highlight = false })
        ...
        Library:ApplyGlassCorner(ListOuter, 5)
        ...
        Library:ApplyGlassCorner(ListInner, 5, { Highlight = false })
        ...
        function Dropdown:BuildDropdownList()
            ...
            for Idx, Value in next, Values do
                ...
                Library:ApplyGlassCorner(Button, 3, { Highlight = false })
...
do
    ...
    Library:ApplyGlassCorner(WatermarkOuter, 6)
    ...
    Library:ApplyGlassCorner(WatermarkInner, 6, { Highlight = false })
    ...
    Library:ApplyGlassCorner(InnerFrame, 5, { Highlight = false })
    ...
    Library:ApplyGlassCorner(KeybindOuter, 6)
    ...
    Library:ApplyGlassCorner(KeybindInner, 6, { Highlight = false })
...
function Library:SpawnNotify(Text, Time)
    ...
    Library:ApplyGlassCorner(GradientFrame, 5, { Highlight = false })
...
function Library:CreateWindow(...)
    ...
    Library:ApplyGlassCorner(Outer, 10)
    ...
    Library:ApplyGlassCorner(Inner, 9, { Highlight = false })
    Library:AddToRegistry(Inner, {
        BackgroundColor3 = 'MainColor';
        BorderColor3 = 'OutlineColor';
    });
    ...
    Library:ApplyGlassCorner(TabBarOuter, 6)
    ...
    Library:ApplyGlassCorner(TabBarInner, 6, { Highlight = false })
    ...
    Library:ApplyGlassCorner(MainSectionOuter, 6)
    ...
    Library:ApplyGlassCorner(MainSectionInner, 6, { Highlight = false })
    ...
    Library:ApplyGlassCorner(TabContainer, 6)
    ...
    -- white by default; darken on hover so user knows it's the resize grip
    local ResizeDefault = Color3.fromRGB(255, 255, 255);
    local ResizeHover = Color3.fromRGB(160, 160, 160);
    CornerCircle.MouseEnter:Connect(function()
        CornerCircle.BackgroundColor3 = ResizeHover;
        CornerCircle.BackgroundTransparency = 0.15;
    end);
    CornerCircle.MouseLeave:Connect(function()
        CornerCircle.BackgroundColor3 = Library.AccentColor or ResizeDefault;
        if CornerCircle.BackgroundColor3.R > 0.9 then
            CornerCircle.BackgroundColor3 = ResizeDefault;
        end
        CornerCircle.BackgroundTransparency = 0.35;
    end);
            ...
            ResizeConn = InputService.InputChanged:Connect(function(Change)
                ...
                if Library.WireframeDrag then
                    if not HasMoved then
                        ...
                        Library:ApplyGlassCorner(Wireframe, 10)
    ...
    function Window:AddTab(Name)
        ...
        Library:ApplyGlassCorner(TabButton, 5)
        ...
        function Tab:AddGroupbox(Info)
            ...
            Library:ApplyGlassCorner(BoxOuter, 7)
            ...
            Library:ApplyGlassCorner(BoxInner, 6, { Highlight = false })
        ...
        function Tab:AddTabbox(Info)
            ...
            function Tabbox:AddTab(Name)
                ...
                Library:ApplyGlassCorner(Button, 4)
                Library:AddToRegistry(Button, {
                    BackgroundColor3 = 'MainColor';
                });
    ...
    pcall(function()
        ...
        Library.GlassTransparency = Library.GlassTransparency or 0.55
        Library:SetGlass(true, Library.GlassTransparency)
    end)
    task.defer(function()
        pcall(function()
            if not Library._LogoGui then
                Library:CreateLogo({ Text = "RCLR", TextSize = 64, SpinSpeed = 55 })
            end
...
if InputService.TouchEnabled then
    ...
    local function CreateMobileButton(name, text, startPos)
        ...
        Library:ApplyGlassCorner(Outer, 6, { Highlight = false })
...
-- ============================================================
-- IMPROVED GLASS: properly bypasses the game background.
-- We create a real viewport-copy stack behind every glass surface,
-- forcing the BlurEffect to sample the live 3D scene, then overlay
-- the UI on top. Also adds specular highlights + rounded corners
-- to every glass surface for that clean iOS/macOS look.
-- ============================================================
function Library:SetGlass(Enabled, Transparency)
    ...
    if type(Transparency) == "number" then
        Library.GlassTransparency = math.clamp(Transparency, 0.15, 0.85)
    end
    Library.GlassTransparency = Library.GlassTransparency or 0.55

    local on = Library.GlassEnabled == true
    local t = Library.GlassTransparency
    -- strong visible layers
    local L_outer = on and math.clamp(t + 0.12, 0.25, 0.82) or 0
    local L_main  = on and math.clamp(t, 0.2, 0.7) or 0
    local L_box   = on and math.clamp(t * 0.65, 0.12, 0.5) or 0

    Library.OuterGlassTransparency = L_outer
    Library._GlassLayer1 = L_outer
    Library._GlassLayer2 = L_main
    Library._GlassLayer3 = L_box
    ...
    pcall(function()
        ...
        for _, d in ipairs(Library.ScreenGui:GetDescendants()) do
            if d:IsA("Frame") or d:IsA("ScrollingFrame") then
                ...
                elseif d == Library.WindowOuter or nm == "Outer" then
                    ...
                    local bgProp = reg and reg.Properties and reg.Properties.BackgroundColor3
                    if bgProp == "BackgroundColor" then
                        d.BackgroundColor3 = Library.BackgroundColor
                        d.BackgroundTransparency = L_box
                    elseif bgProp == "MainColor" then
                        d.BackgroundColor3 = Library.MainColor
                        d.BackgroundTransparency = L_main
                    elseif on and d.BackgroundTransparency < 0.08 then
                        -- any other solid panel
                        if d.BorderSizePixel >= 0 and d.AbsoluteSize.Y >= 8 and d.AbsoluteSize.X >= 8 then
                            if not nm:find("Btn") and not nm:find("Display") then
                                d.BackgroundTransparency = math.min(L_box, 0.4)
                            end
                        end
                    end
                end
        ...
        -- Re-apply glass highlight on every glass frame so it always
        -- renders the specular sheen (this is what sells the "glass" look).
        for _, d in ipairs(Library.ScreenGui:GetDescendants()) do
            if d:IsA("Frame") and d:FindFirstChild('__GlassCorner') then
                local hl = d:FindFirstChild('__GlassHighlight')
                if hl then
                    hl.BackgroundTransparency = on and 0.92 or 1
                end
            end
        end
        ...
        if on then
            ...
            Library.BlurSize = math.clamp(Library.BlurSize or 32, 12, 48)
...
function Library:CreateLogo(Info)
    ...
    local speed = tonumber(Info.SpinSpeed) or 55
...
function Library:UpdateLogoVisibility()
    if Library._LogoSync then pcall(Library._LogoSync)
    elseif Library._LogoGui then Library._LogoGui.Enabled = Library.Toggled == true end
end


-- ============================================================
-- REDESIGNED LIVE STUD UI
-- Clean card layout, animated accent bar, monospaced readouts,
-- subtle pulse on the border, and a live mini-map dot.
-- ============================================================
function Library:CreateLiveStudUI(Info)
    Info = Info or {}
    local Title = tostring(Info.Title or "RCLR_Monova")
    local Side = Info.Side or "Left"
    local Rate = tonumber(Info.UpdateRate) or 0.05
    local W, H = 178, 128

    pcall(function()
        if Library.LiveStudUI and Library.LiveStudUI.Destroy then Library.LiveStudUI:Destroy() end
    end)

    local Gui = Instance.new("ScreenGui")
    Gui.Name = "RCLR_LiveStud"
    Gui.ResetOnSpawn = false
    Gui.DisplayOrder = 50
    pcall(function() ProtectGui(Gui) end)
    pcall(function() Gui.Parent = CoreGui end)
    ...
    Card.Name = "Card"
    Card.BackgroundColor3 = Library.BackgroundColor or Color3.fromRGB(12, 12, 12)
    Card.BackgroundTransparency = 0.15
    Card.BorderSizePixel = 0
    Card.Size = UDim2.fromOffset(W, H)
    Card.Position = Side == "Right" and UDim2.new(1, -(W + 12), 0, 90) or UDim2.new(0, 12, 0, 90)
    Card.ZIndex = 80
    Card.Parent = Gui
    ...
    cardStroke.Color = Library.OutlineColor or Color3.fromRGB(50, 50, 50)
    ...
    cardGrad.Color = ColorSequence.new({
        ...
        ColorSequenceKeypoint.new(1, Library.BackgroundColor or Color3.fromRGB(12, 12, 12)),
    })
    ...
    accent.Name = "Accent"
    accent.BackgroundColor3 = Library.AccentColor or Color3.fromRGB(255, 255, 255)
    ...
    title.TextSize = 11
    title.TextColor3 = Library.FontColor or Color3.fromRGB(255, 255, 255)
    ...
    sep.BackgroundColor3 = Library.OutlineColor or Color3.fromRGB(50, 50, 50)
    sep.BackgroundTransparency = 0.4
    sep.BorderSizePixel = 0
    sep.Position = UDim2.new(0, 10, 0, 26)
    sep.Size = UDim2.new(1, -20, 0, 1)
    sep.ZIndex = 82
    sep.Parent = Card

    -- === Axis rows ===
    local function makeRow(y, name, dotColor)
        ...
        tag.BackgroundColor3 = dotColor
        tag.BorderSizePixel = 0
        tag.Size = UDim2.fromOffset(3, 12)
        tag.Position = UDim2.new(0, 0, 0.5, -6)
        tag.ZIndex = 83
        tag.Parent = row
        ...
        nameLabel.Size = UDim2.fromOffset(20, 22)
        nameLabel.Font = Enum.Font.GothamBold
        nameLabel.TextSize = 11
        nameLabel.TextColor3 = dotColor
        ...
        value.Size = UDim2.fromOffset(100, 22)
        value.Font = Enum.Font.Code
        value.TextSize = 14
        value.TextColor3 = Library.FontColor or Color3.fromRGB(255, 255, 255)
    ...
    local X_val, X_tag = makeRow(34, "X", Color3.fromRGB(255, 90, 90))
    local Y_val, Y_tag = makeRow(58, "Y", Color3.fromRGB(90, 255, 130))
    local Z_val, Z_tag = makeRow(82, "Z", Color3.fromRGB(90, 150, 255))

    -- === Footer: speed/velocity readout ===
    local footer = Instance.new("Frame")
    footer.BackgroundColor3 = Library.MainColor or Color3.fromRGB(22, 22, 22)
    ...
    speedLabel.TextColor3 = Library.AccentColor or Color3.fromRGB(255, 255, 255)
    speedLabel.TextXAlignment = Enum.TextXAlignment.Left
    speedLabel.Text = "VEL 0.0"
    speedLabel.ZIndex = 82
    speedLabel.Parent = footer

    pcall(function() Library:MakeDraggable(Card, 24, false) end)
    ...
    local conn = RunService.Heartbeat:Connect(function(dt)
        -- border pulse
        pulse = (pulse + dt) % 2
        local p = math.abs(math.sin(pulse * math.pi))
        cardStroke.Transparency = 0.25 + p * 0.25
        ...
        -- Theme sync
        Card.BackgroundColor3 = Library.BackgroundColor or Card.BackgroundColor3
        cardStroke.Color = Library.OutlineColor or cardStroke.Color
        accent.BackgroundColor3 = Library.AccentColor or accent.BackgroundColor3
        title.TextColor3 = Library.FontColor or title.TextColor3
        footer.BackgroundColor3 = Library.MainColor or footer.BackgroundColor3
        speedLabel.TextColor3 = Library.AccentColor or speedLabel.TextColor3
        X_val.TextColor3 = Library.FontColor or X_val.TextColor3
        Y_val.TextColor3 = Library.FontColor or Y_val.TextColor3
        Z_val.TextColor3 = Library.FontColor or Z_val.TextColor3

        local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        if hrp then
            local pos = hrp.Position
            X_val.Text = string.format("%.0f", pos.X)
            Y_val.Text = string.format("%.0f", pos.Y)
            Z_val.Text = string.format("%.0f", pos.Z)

            -- speed
            if lastPos then
                local delta = (pos - lastPos).Magnitude
                local spd = delta / Rate
                speedLabel.Text = string.format("VEL %.1f", spd)
            end
            lastPos = pos

            dot.BackgroundColor3 = Color3.fromRGB(80, 255, 120)
        else
            X_val.Text, Y_val.Text, Z_val.Text = "---", "---", "---"
            speedLabel.Text = "VEL ---"
    ...
    function API:SetVisible(v) Card.Visible = not not v; Gui.Enabled = not not v end
    function API:Destroy()
        pcall(function() conn:Disconnect() end)
        pcall(function() Gui:Destroy() end)
        if Library.LiveStudUI == API then Library.LiveStudUI = nil end
    end
    Library.LiveStudUI = API
    return API
end
