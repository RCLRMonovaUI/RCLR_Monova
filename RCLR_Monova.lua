...
function Library:SpawnNotify(Text, Time)
    ...
    TweenService:Create(Outer, TweenInfo.new(0.28, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
        Size = UDim2.fromOffset(xw + 16, H);
    }):Play();
    Library.ActiveNotifyCount = Library.ActiveNotifyCount + 1;
    task.spawn(function()
        task.wait(Time or 5);
        local t = TweenService:Create(Outer, TweenInfo.new(0.22, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
            Size = UDim2.fromOffset(0, H);
        });
        t:Play();
        t.Completed:Wait();
...
function Library:CreateWindow(...)
    ...
    pcall(function()
        ...
        Library:SetGlass(true, Library.GlassTransparency or 0.65)
    end)
    task.defer(function()
        pcall(function()
...
function Library:SetGlass(Enabled, Transparency)
    ...
    Library.GlassTransparency = Library.GlassTransparency or 0.7
    local on = Library.GlassEnabled == true
    local t = Library.GlassTransparency
    -- real glass: high transparency so world shows through
    local L_outer = on and math.clamp(t + 0.08, 0.48, 0.9) or 0
    local L_main  = on and math.clamp(t * 0.95, 0.4, 0.85) or 0
    local L_box   = on and math.clamp(t * 0.8, 0.32, 0.72) or 0
    local L_slide = on and math.clamp(t * 0.5, 0.22, 0.48) or 0
    Library.OuterGlassTransparency = L_outer

    pcall(function()
        if on then
            Library.UseDarken = true
            Library.DarkenAmount = 15
            Library.UseBlur = true
            Library.BlurSize = math.clamp(Library.BlurSize or 12, 4, 20)
        end
        ...
        for _, d in ipairs(Library.ScreenGui:GetDescendants()) do
            if d:IsA("Frame") or d:IsA("ScrollingFrame") then
                ...
                elseif pname == "TabArea" then
                    -- tab buttons solid
                    d.BackgroundTransparency = 0
                elseif nm:find("Fill") or nm:find("Accent") or nm:find("Corner") or nm:find("Cursor") or nm:find("Indicator") or nm:find("Highlight") then
