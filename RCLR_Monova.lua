...
function Library:SpawnNotify(Text, Time)
    ...
    -- cooler open: expand width + slight bounce
    local openTw = TweenService:Create(Outer, TweenInfo.new(0.32, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
        Size = UDim2.fromOffset(xw + 18, H);
    });
    openTw:Play();
    Library.ActiveNotifyCount = Library.ActiveNotifyCount + 1;
    task.spawn(function()
        task.wait(Time or 4.5);
        -- cooler close: shrink + fade children
        for _, ch in ipairs(Outer:GetDescendants()) do
            if ch:IsA("Frame") then
                TweenService:Create(ch, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
                    BackgroundTransparency = 1;
                }):Play()
        ...
        local closeTw = TweenService:Create(Outer, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
            Size = UDim2.fromOffset(0, H);
        });
        closeTw:Play();
...
function Library:CreateWindow(...)
    ...
    local function RefreshTabCanvas()
        pcall(function()
            local w = TabListLayout.AbsoluteContentSize.X + 4;
            if w < 1 then w = 1 end
    ...
    -- drag-slide tabs (threshold so clicks still work)
    do
    ...
    function Library:Toggle()
        ...
        if opening then
            ...
            TweenService:Create(Outer, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
                Size = fullSize;
                BackgroundTransparency = (Library.GlassEnabled and (Library.OuterGlassTransparency or 0.5)) or 0;
            }):Play();
            task.defer(function()
            ...
            local tw = TweenService:Create(Outer, TweenInfo.new(0.16, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
                Size = UDim2.fromOffset(
                    math.max(40, fullSize.X.Offset * 0.92),
                    math.max(40, fullSize.Y.Offset * 0.92)
                );
            ...
            tw.Completed:Connect(function()
                if not Library.Toggled then
                    Outer.Visible = false;
                    Outer.Size = fullSize;
                    Outer.BackgroundTransparency = (Library.GlassEnabled and (Library.OuterGlassTransparency or 0.5)) or 0;
                end
            end)
            pcall(function()
    ...
    pcall(function()
        ...
        Library:SetGlass(true, Library.GlassTransparency or 0.78)
    end)
    task.defer(function()
        pcall(function()
            if not Library._LogoGui then
                Library:CreateLogo({ Text = "RCLR", TextSize = 64, SpinSpeed = 45 })
            end
        ...
        for _, d in ipairs(Library.ScreenGui:GetDescendants()) do
            if d:IsA("Frame") or d:IsA("ScrollingFrame") then
                ...
                elseif pname == "TabArea" then
                    d.BackgroundTransparency = 0
                    -- keep registry colors (MainColor / BackgroundColor)
