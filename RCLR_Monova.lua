...
-- PC / Mobile performance profile
do
    local mobile = false
    pcall(function()
        mobile = InputService.TouchEnabled and not InputService.KeyboardEnabled
        if InputService.TouchEnabled and InputService.GyroscopeEnabled then mobile = true end
        if InputService.TouchEnabled then mobile = true end
    end)
...
function Library:UpdateBlur()
    ...
    local useBlur = open and Library.UseBlur and not Library.IsMobile
...
table.insert(Library.Signals, RunService.Heartbeat:Connect(function(Delta)
    ...
    local interval = Library.IsMobile and (1 / 20) or (1 / 30)
...
function Library:CreateWindow(...)
    ...
    pcall(function() if Library.SetGlass then Library:SetGlass(true, Library.GlassTransparency or 0.78) end end)
...
function Library:ReapplyGlass()
    Library:SetGlass(Library.GlassEnabled ~= false, Library.GlassTransparency or 0.78)
end
...
function Library:CreateLogo(Info)
    ...
    local angle = 0
    local acc = 0
    local step = Library.IsMobile and (1/20) or (1/30)
