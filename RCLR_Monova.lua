--[[
  RCLR Monova 5 - FIXED (2026-09-24)
  - Slider label auto-contrasts (visible on white/light accents)
  - Tab bar: mouse wheel + drag both scroll
  - Glass untouched (Monova-5 glass preserved)
  Force refresh: ?v=6
]]
...
-- [NEW] Contrast helper: dark text on light accent, light text on dark accent
...
function Library:UpdateColorsUsingRegistry()
    ...
    -- [NEW] Refresh every slider label so it stays readable on any accent
...
do
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        -- [FIXED] Raw TextLabel (NOT CreateLabel) with contrast color
        -- so the % text stays visible whether accent is white, black, red, etc.
        ...
        -- Exposed so Library:UpdateColorsUsingRegistry can refresh contrast on theme change
...
function Library:CreateWindow(...)
    ...
    -- [FIXED] Tab wheel + drag both work
    pcall(function()
        TabArea.ScrollingEnabled = false -- manual drag handling instead
    end)
    do
        ...
        TabArea.InputChanged:Connect(function(input)
            ...
            if input.UserInputType == Enum.UserInputType.MouseWheel then
                local maxScroll = math.max(0, TabListLayout.AbsoluteContentSize.X - TabArea.AbsoluteSize.X + 8)
                local step = (input.Position.Z or 0) * 50
                local newX = math.clamp(TabArea.CanvasPosition.X - step, 0, maxScroll)
                TabArea.CanvasPosition = Vector2.new(newX, 0)
                return
            end
            -- Drag scroll
            ...
            if input.UserInputType == Enum.UserInputType.MouseMovement
                ...
                if math.abs(dx) > dragThreshold then
                    local maxScroll = math.max(0, TabListLayout.AbsoluteContentSize.X - TabArea.AbsoluteSize.X + 8)
                    TabArea.CanvasPosition = Vector2.new(math.clamp(startCanvas + dx, 0, maxScroll), 0)
                end
            end
        end)
...
-- =========================================================
-- GLASS FUNCTIONS (KEPT EXACTLY AS-IS)
-- =========================================================
