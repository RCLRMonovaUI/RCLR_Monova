...
function Library:CreateWindow(...)
    ...
    -- if tabs overflow, shift list so latest stay reachable (simple, no gray bar)
    local function RefreshTabCanvas()
        pcall(function()
            local need = TabListLayout.AbsoluteContentSize.X
            local have = TabArea.AbsoluteSize.X
            if need > have + 2 then
                TabListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Left
                -- keep tab buttons visible; parent already clips
            end
        end)
    end
    ...
    -- hover: slightly darker so you know you can resize
    ...
    function Library:Toggle()
        ...
        if opening then
            ...
            -- menu closed: kill logo overlay leftovers
            pcall(function()
                if Library._LogoGui then Library._LogoGui.Enabled = false end
            end)
        end;
...
function Library:UpdateLogoVisibility()
    ...
    -- never leave logo visible when menu closed
