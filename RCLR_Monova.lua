...
function Library:SetGlass(Enabled, Transparency)
    ...
    if type(Transparency) == "number" then
        Library.GlassTransparency = math.clamp(Transparency, 0.45, 0.95)
    end
    ...
    -- Glass levels (higher = more transparent = more glass)
    local L_outer = on and math.clamp(t + 0.05, 0.65, 0.95) or 0
    local L_box   = on and math.clamp(t * 0.85, 0.45, 0.85) or 0
    local L_main  = on and math.clamp(t * 0.65, 0.35, 0.65) or 0
    ...
    pcall(function()
        ...
        for _, d in ipairs(Library.ScreenGui:GetDescendants()) do
            if d:IsA("Frame") or d:IsA("ScrollingFrame") then
                ...
                -- Never touch structural tab bars
                if nm == "TabArea" then
