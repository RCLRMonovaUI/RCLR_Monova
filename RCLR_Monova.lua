...
do
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        function Slider:Display()
            ...
            TweenService:Create(Fill, TweenInfo.new(0.18, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
...
function Library:CreateWindow(...)
    ...
    local function RefreshTabCanvas()
        pcall(function()
            local w = TabListLayout.AbsoluteContentSize.X + 8
            TabArea.CanvasSize = UDim2.fromOffset(math.max(w, 0), 0)
        end)
    end
    TabListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(RefreshTabCanvas)
    -- mouse drag / wheel scroll tabs horizontally
    do
        ...
        TabArea.InputChanged:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseWheel then
                TabArea.CanvasPosition = Vector2.new(
                    math.max(0, TabArea.CanvasPosition.X - input.Position.Z * 40),
                    0
                )
            end
        end)
    ...
    -- inner dim circle for cleaner grip look
    local CornerInner = Library:Create('Frame', {
        AnchorPoint = Vector2.new(0.5, 0.5);
    ...
    local ResizeHover = Color3.fromRGB(140, 140, 140);
    CornerCircle.MouseEnter:Connect(function()
        CornerCircle.BackgroundColor3 = ResizeHover;
        CornerCircle.BackgroundTransparency = 0.05;
        CornerInner.BackgroundTransparency = 0.2;
    end);
    CornerCircle.MouseLeave:Connect(function()
        CornerCircle.BackgroundColor3 = Library.AccentColor or ResizeDefault;
        if CornerCircle.BackgroundColor3.R > 0.85 and CornerCircle.BackgroundColor3.G > 0.85 then
            CornerCircle.BackgroundColor3 = ResizeDefault;
        end
        CornerCircle.BackgroundTransparency = 0.25;
        CornerInner.BackgroundTransparency = 0.4;
    end);
    ...
    function Window:AddTab(Name)
        ...
        TabButtonLabel.Text = tostring(Name);
        ...
        pcall(function()
            ...
            if RefreshTabCanvas then RefreshTabCanvas() end
        end);
    ...
    task.defer(function()
        ...
        pcall(function()
            if Library.CreateLiveStudUI and not Library.LiveStudUI then
                Library:CreateLiveStudUI({ Title = "RCLR_Monova", Side = "Left", UpdateRate = 0.08 })
            end
        end)
        pcall(function()
            if Window._RefreshTabCanvas then Window._RefreshTabCanvas() end
...
if InputService.TouchEnabled then
    ...
    local function CreateMobileButton(name, text, startPos)
        -- original Linoria-style mobile buttons
        local Outer = Library:Create('Frame', {
            Name             = name .. "Outer",
...
function Library:SetGlass(Enabled, Transparency)
    -- Persistent see-through glass (stays after close/open)
    ...
    if type(Transparency) == "number" then
        Library.GlassTransparency = math.clamp(Transparency, 0.2, 0.92)
    end
    Library.GlassTransparency = Library.GlassTransparency or 0.68
    Library._GlassWanted = Library.GlassEnabled

    local on = Library.GlassEnabled == true
    local t = Library.GlassTransparency
    local L_outer = on and math.clamp(t + 0.1, 0.42, 0.9) or 0
    local L_main  = on and math.clamp(t, 0.38, 0.85) or 0
    local L_box   = on and math.clamp(t * 0.88, 0.32, 0.75) or 0
    local L_slide = on and math.clamp(t * 0.55, 0.2, 0.5) or 0

    Library.OuterGlassTransparency = L_outer
    Library._GlassLayer1 = L_outer
    Library._GlassLayer2 = L_main
    Library._GlassLayer3 = L_box
    Library._GlassSlider = L_slide

    pcall(function()
        if on then
            Library.UseDarken = true
            Library.DarkenAmount = 18
            Library.UseBlur = true
            Library.BlurSize = math.clamp(Library.BlurSize or 14, 6, 22)
        end
        ...
        for _, d in ipairs(Library.ScreenGui:GetDescendants()) do
            if d:IsA("Frame") or d:IsA("ScrollingFrame") then
                ...
                local pname = d.Parent and tostring(d.Parent.Name or "") or ""

                if nm:find("Fill") or nm:find("Accent") or nm:find("Corner")
                    or nm:find("Cursor") or nm:find("Indicator") or nm:find("Highlight")
                    or nm:find("Blocker") or nm:find("Logo") then
                    -- solid accents
                elseif nm == "TabBarOuter" or nm == "TabBarInner" or nm == "TabArea" or pname == "TabArea" then
                ...
                elseif d == Library.WindowOuter or nm == "Outer" then
                    ...
                    -- light glass on slider tracks (MainColor bars)
                    if bgProp == "MainColor" then
                        d.BackgroundColor3 = Library.MainColor
                        d.BackgroundTransparency = L_slide
                    elseif bgProp == "BackgroundColor" then
                        d.BackgroundColor3 = Library.BackgroundColor
                        d.BackgroundTransparency = L_box
                    elseif on and d.BackgroundTransparency < 0.18 then
                        if d.AbsoluteSize.Y >= 8 and d.AbsoluteSize.X >= 8 then
                            if not nm:find("Btn") and not nm:find("Display") then
                                d.BackgroundTransparency = math.min(L_box, 0.55)
                            end
                        end
                    end
                end
...
function Library:ReapplyGlass()
    -- always restore last glass settings (even after close)
    local want = Library._GlassWanted
    if want == nil then want = Library.GlassEnabled ~= false end
    Library.GlassEnabled = want
    Library.GlassTransparency = Library.GlassTransparency or 0.68
...
function Library:CreateLiveStudUI(Info)
    ...
    local Rate = tonumber(Info.UpdateRate) or 0.08
    local W, H = 168, 118

    pcall(function()
        if Library.LiveStudUI and Library.LiveStudUI.Destroy then
            Library.LiveStudUI:Destroy()
        end
    end)
    ...
    Shell.Name = "StudShell"
    Shell.BackgroundColor3 = Library.OutlineColor or Color3.fromRGB(50, 50, 50)
    Shell.BackgroundTransparency = 0.35
    Shell.BorderSizePixel = 0
    Shell.Size = UDim2.fromOffset(W, H)
    Shell.Position = Side == "Right" and UDim2.new(1, -(W + 16), 0, 90) or UDim2.new(0, 16, 0, 90)
    Shell.ZIndex = 80
    Shell.Parent = Gui

    local Frame = Instance.new("Frame")
    Frame.Name = "StudRoot"
    Frame.BackgroundColor3 = Library.BackgroundColor or Color3.fromRGB(12, 12, 12)
    ...
    top.BackgroundColor3 = Library.AccentColor or Color3.fromRGB(255, 255, 255)
    ...
    rule.BackgroundColor3 = Library.AccentColor or Color3.fromRGB(255, 255, 255)
    rule.BackgroundTransparency = 0.55
    rule.BorderSizePixel = 0
    rule.Position = UDim2.fromOffset(10, 28)
    rule.Size = UDim2.new(1, -20, 0, 1)
    rule.ZIndex = 82
    rule.Parent = Frame

    local function axis(y, name)
        ...
        line.BackgroundColor3 = Library.AccentColor or Color3.fromRGB(255, 255, 255)
        line.BorderSizePixel = 0
        line.Position = UDim2.fromOffset(10, 4)
        line.Size = UDim2.fromOffset(2, 12)
        line.ZIndex = 84
        line.Parent = row

        local lab = Instance.new("TextLabel")
        lab.BackgroundTransparency = 1
        lab.Position = UDim2.fromOffset(18, 2)
        lab.Size = UDim2.fromOffset(16, 16)
        lab.Font = Enum.Font.Code
        lab.TextSize = 13
        lab.TextColor3 = Library.FontColor or Color3.fromRGB(255, 255, 255)
        ...
        chip.BackgroundColor3 = Library.MainColor or Color3.fromRGB(22, 22, 22)
        chip.BackgroundTransparency = 0.25
        chip.BorderSizePixel = 0
        chip.Position = UDim2.new(1, -86, 0, 2)
        chip.Size = UDim2.fromOffset(76, 16)
        chip.ZIndex = 84
        chip.Parent = row
        ...
        val.TextSize = 12
        val.TextColor3 = Library.FontColor or Color3.fromRGB(255, 255, 255)
        val.TextXAlignment = Enum.TextXAlignment.Center
        val.Text = "0.0"
        val.ZIndex = 85
        val.Parent = chip

        return val, line, lab, chip
    end

    local xV, xL, xLab, xC = axis(36, "X")
    local yV, yL, yLab, yC = axis(58, "Y")
    local zV, zL, zLab, zC = axis(80, "Z")

    pcall(function() Library:MakeDraggable(Shell, 28, false) end)
    ...
    local conn = RunService.Heartbeat:Connect(function(dt)
        ...
        local bg = Library.BackgroundColor or Color3.fromRGB(12, 12, 12)
        local main = Library.MainColor or Color3.fromRGB(22, 22, 22)
        local ac = Library.AccentColor or Color3.fromRGB(255, 255, 255)
        local fc = Library.FontColor or Color3.fromRGB(255, 255, 255)
        local ol = Library.OutlineColor or Color3.fromRGB(50, 50, 50)

        Shell.BackgroundColor3 = ol
        Frame.BackgroundColor3 = bg
        -- slight glass on stud if library glass on
        if Library.GlassEnabled then
            Frame.BackgroundTransparency = 0.28
            Shell.BackgroundTransparency = 0.45
        else
            Frame.BackgroundTransparency = 0.12
            Shell.BackgroundTransparency = 0.3
        end
        top.BackgroundColor3 = ac
        rule.BackgroundColor3 = ac
        title.TextColor3 = fc
        xL.BackgroundColor3, yL.BackgroundColor3, zL.BackgroundColor3 = ac, ac, ac
        xV.TextColor3, yV.TextColor3, zV.TextColor3 = fc, fc, fc
        xLab.TextColor3, yLab.TextColor3, zLab.TextColor3 = fc, fc, fc
        xC.BackgroundColor3, yC.BackgroundColor3, zC.BackgroundColor3 = main, main, main

        local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        if hrp then
            local p = hrp.Position
            xV.Text = string.format("%.1f", p.X)
            yV.Text = string.format("%.1f", p.Y)
            zV.Text = string.format("%.1f", p.Z)
        else
            xV.Text, yV.Text, zV.Text = "---", "---", "---"
        end
    end)
    table.insert(Library.Signals, conn)
    ...
    function API:SetVisible(v)
        Shell.Visible = not not v
        Gui.Enabled = not not v
    end
