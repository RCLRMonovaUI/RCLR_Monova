--[[
  RCLR Monova - FIXED (2026-09-24)
  - Restored Monova-5 glass
  - Tabs scrollable (wheel + drag)
  - Slider fill matches theme, label auto-contrasts
  Force refresh: ?v=5
]]
...
-- Helper: contrast color for text over a given color
...
function Library:UpdateColorsUsingRegistry()
    ...
    -- Refresh slider labels that depend on accent contrast
...
do
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        -- FIX: fill uses AccentColor with a subtle gradient so it's not blinding
        ...
        -- FIX: slider label auto-contrasts against accent so % is always readable
        local DisplayLabel = Library:CreateLabel({
            Size = UDim2.new(1, 0, 1, 0);
            TextSize = Library.FontSize;
            Text = 'Infinite';
        ...
        -- Strengthen the stroke for readability
        for _, child in ipairs(DisplayLabel:GetChildren()) do
...
function Library:CreateWindow(...)
    ...
    pcall(function()
        ...
        -- FIX: allow wheel + drag scrolling on tabs
        TabArea.ScrollingEnabled = true
        TabArea.ScrollBarThickness = 0
        TabArea.ElasticBehavior = Enum.ElasticBehavior.Never
    end);
    ...
    -- FIX: drag tabs works AND wheel works now
    do
        ...
        InputService.InputChanged:Connect(function(input)
            ...
            if input.UserInputType == Enum.UserInputType.MouseMovement
                ...
                if math.abs(dx) > dragThreshold then
                    hasDragged = true
