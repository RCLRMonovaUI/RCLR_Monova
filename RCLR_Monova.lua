...
function Library:AddCorner(Inst, Radius)
    ...
    if Inst:FindFirstChildOfClass("UICorner") then
        Inst:FindFirstChildOfClass("UICorner").CornerRadius = UDim.new(0, Radius or Library.CornerRadius)
        return
    end
...
-- global heartbeat to fix ScrollingFrames canvas sizing (tab content)
...
function Library:SpawnNotify(Text, Time)
    ...
    -- Cooler animation: slide + fade + slight scale
...
function Library:CreateWindow(...)
    ...
    function Window:AddTab(Name)
        ...
        -- redundant safety: also update canvas on content change
...
function Library:SetGlass(Enabled, Transparency)
    ...
    -- high transparency = more game visible behind
    ...
    pcall(function()
        if on then
            -- real glass: blur the world so it reads as frosted, don't blanket-darken
...
function Library:CreateLogo(Info)
    ...
    -- pendulum swing: goes to +60, comes back through 0 to -60, back
    local t = 0
    local swingRange = 60  -- degrees each side
    local swingSpeed = speed / 40  -- normalize to ~1 cycle per 3 seconds at 40
