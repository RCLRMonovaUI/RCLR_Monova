...
function Library:SetGlass(Enabled, Transparency)
    ...
    local L1 = on and math.clamp(t + 0.1, 0.4, 0.88) or 0
    local L2 = on and math.clamp(t, 0.35, 0.8) or 0
    local L3 = on and math.clamp(t * 0.85, 0.28, 0.7) or 0
    pcall(function()
        if on then
            Library.UseDarken = true
            Library.DarkenAmount = 20
        ...
        for _, d in ipairs(Library.ScreenGui:GetDescendants()) do
            if d:IsA("Frame") or d:IsA("ScrollingFrame") then
                ...
                elseif nm:find("Fill") or nm:find("Accent") or nm:find("Corner") or nm:find("Cursor") or nm:find("Indicator") then
                elseif d == Library.WindowOuter or nm == "Outer" then
                    ...
                    elseif bg == "MainColor" then
                        ...
                        d.BackgroundTransparency = math.min(L2, 0.45)
                    end
                end
            end
        end
