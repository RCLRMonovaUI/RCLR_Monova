...
if InputService.TouchEnabled then
    ...
    local function CreateMobileButton(name, text, startPos)
        -- simple black box + white border (screenshot style)
        local Outer = Library:Create('Frame', {
            Name             = name .. "Outer",
...
function Library:SetGlass(Enabled, Transparency)
    -- See-through glass: game world visible behind the whole UI
    ...
    if type(Transparency) == "number" then
        Library.GlassTransparency = math.clamp(Transparency, 0.25, 0.9)
    end
    Library.GlassTransparency = Library.GlassTransparency or 0.65

    local on = Library.GlassEnabled == true
    local t = Library.GlassTransparency
    local L_outer = on and math.clamp(t + 0.12, 0.4, 0.9) or 0
    local L_main  = on and math.clamp(t, 0.35, 0.85) or 0
    local L_box   = on and math.clamp(t * 0.9, 0.3, 0.75) or 0
    ...
    pcall(function()
        if on then
            ...
            Library.DarkenAmount = 22
            Library.UseBlur = true
            Library.BlurSize = math.clamp(Library.BlurSize or 16, 6, 24)
        end
        ...
        for _, d in ipairs(Library.ScreenGui:GetDescendants()) do
            if d:IsA("Frame") or d:IsA("ScrollingFrame") then
                ...
                elseif d == Library.WindowOuter or nm == "Outer" then
                    ...
                    elseif on and d.BackgroundTransparency < 0.2 then
                        if d.AbsoluteSize.Y >= 10 and d.AbsoluteSize.X >= 10 then
...
function Library:CreateLiveStudUI(Info)
    ...
    Frame.Position = Side == "Right" and UDim2.new(1, -(W + 14), 0, 88) or UDim2.new(0, 14, 0, 88)
    Frame.ZIndex = 80
    Frame.Parent = Gui

    local stroke = Instance.new("UIStroke")
    stroke.Color = Library.OutlineColor or Color3.fromRGB(55, 55, 55)
    stroke.Thickness = 1
    ...
    sep.Position = UDim2.fromOffset(8, 28)
    ...
    local function axis(y, name)
        ...
        line.Position = UDim2.fromOffset(10, 3)
        ...
        lab.Position = UDim2.fromOffset(18, 1)
        lab.Size = UDim2.fromOffset(18, 16)
        ...
        val.Position = UDim2.new(1, -78, 0, 1)
        val.Size = UDim2.fromOffset(68, 16)
    ...
    local xV, xL, xLab = axis(34, "X")
    local yV, yL, yLab = axis(54, "Y")
    local zV, zL, zLab = axis(74, "Z")

    pcall(function() Library:MakeDraggable(Frame, 26, false) end)
    ...
    local conn = RunService.Heartbeat:Connect(function(dt)
        ...
        Frame.BackgroundColor3 = Library.BackgroundColor or Frame.BackgroundColor3
        stroke.Color = Library.OutlineColor or stroke.Color
        top.BackgroundColor3 = Library.AccentColor or top.BackgroundColor3
        sep.BackgroundColor3 = Library.OutlineColor or sep.BackgroundColor3
