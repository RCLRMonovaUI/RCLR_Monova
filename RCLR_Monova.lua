...
local Library = {
    ...
    -- [LOCK] Flag that the mobile Lock button toggles
    UILocked = false;

    -- [GLASS] Global config for the glass look
    GlassEnabled         = true,
    GlassTransparency    = 0.10,   -- Inner background transparency (0 = opaque, 1 = invisible)
    GlassPanelAlpha      = 0.72,   -- transparency of the frosted glass panels
    GlassEdgeColor       = Color3.fromRGB(255, 255, 255),
    GlassEdgeTransparency= 0.82,   -- transparency of edge highlights
    GlassEdgeThickness   = 1,
...
-- [LOCK] Added lock check for the window drag
function Library:MakeDraggable(Instance, Cutoff, IsWindow)
    Instance.Active = true;
    Instance.InputBegan:Connect(function(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
            -- [LOCK] If the UI is locked, ignore drags on windows
...
do
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        -- [ANIM] Slider Fill tween
        local FillTween
        local function TweenFill(targetX)
            if FillTween then FillTween:Cancel() end
            FillTween = TweenService:Create(Fill,
                TweenInfo.new(0.10, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                { Size = UDim2.new(0, targetX, 1, 0) })
            FillTween:Play()
        end
...
-- [GLASS] Helper: builds the frosted glass panel + edge highlights on a frame
local function BuildGlass(Frame, Properties)
    Properties = Properties or {}

    local Glass = Library:Create('Frame', {
        Name                = 'GlassPanel',
        BackgroundColor3    = Color3.fromRGB(255, 255, 255),
        BackgroundTransparency = Properties.Transparency or Library.GlassPanelAlpha,
        BorderSizePixel     = 0,
    ...
    Library:Create('Frame', {
        Name              = 'GlassTop',
        BackgroundColor3  = Library.GlassEdgeColor,
        BackgroundTransparency = Library.GlassEdgeTransparency,
        BorderSizePixel   = 0,
        Size              = UDim2.new(1, 0, 0, Library.GlassEdgeThickness),
        Position          = UDim2.new(0, 0, 0, 0),
        ZIndex            = (Properties.ZIndex or 0) + 1,
        Parent            = Glass,
    })
    ...
    Library:Create('Frame', {
        Name              = 'GlassBottom',
        BackgroundColor3  = Library.GlassEdgeColor,
        BackgroundTransparency = Library.GlassEdgeTransparency + 0.05,
        BorderSizePixel   = 0,
        Size              = UDim2.new(1, 0, 0, Library.GlassEdgeThickness),
        Position          = UDim2.new(0, 0, 1, -Library.GlassEdgeThickness),
        ZIndex            = (Properties.ZIndex or 0) + 1,
        Parent            = Glass,
    })
    -- Left edge
    Library:Create('Frame', {
        ...
        BackgroundTransparency = Library.GlassEdgeTransparency,
        BorderSizePixel   = 0,
        Size              = UDim2.new(0, Library.GlassEdgeThickness, 1, 0),
        Position          = UDim2.new(0, 0, 0, 0),
        ZIndex            = (Properties.ZIndex or 0) + 1,
        Parent            = Glass,
    })
    ...
    Library:Create('Frame', {
        ...
        BorderSizePixel   = 0,
        Size              = UDim2.new(0, Library.GlassEdgeThickness, 1, 0),
        Position          = UDim2.new(1, -Library.GlassEdgeThickness, 0, 0),
        ZIndex            = (Properties.ZIndex or 0) + 1,
        Parent            = Glass,
    })
...
function Library:CreateWindow(...)
    ...
    if type(Config.Glass) == 'boolean' then Library.GlassEnabled = Config.Glass end
    if type(Config.GlassTransparency) == 'number' then Library.GlassTransparency = Config.GlassTransparency end
    ...
    -- [GLASS] Outer stays black to give contrast to the glass edge
    local Outer = Library:Create('Frame', {
    ...
    -- [GLASS] Frosted glass panel sits behind the inner frame
    local GlassPanel = Library.GlassEnabled and BuildGlass(Outer, { ZIndex = 0, Transparency = Library.GlassPanelAlpha }) or nil
    ...
    -- [SQUARE] Replaced the rounded CornerCircle with a clean square resize handle
    local CornerHandle = Library:Create('Frame', {
        Name              = 'ResizeHandle',
    ...
    Library:AddToRegistry(CornerHandle, {
        BackgroundColor3 = 'AccentColor';
    });
    -- Square dot pattern inside the handle (clean, not rounded)
    for i = 0, 1 do
        for j = 0, 1 do
            Library:Create('Frame', {
                BackgroundColor3 = Color3.fromRGB(255, 255, 255),
                BackgroundTransparency = 0.35,
                BorderSizePixel  = 0,
                Position         = UDim2.new(0.5, -3 + i * 6, 0.5, -3 + j * 6),
    ...
    do
        ...
        CornerHandle.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
    ...
    -- [ANIM] Open / close animation for the main window
    local storedTargetSize = Config.Size
    ...
    local function PlayOpenAnimation()
        ...
        storedTargetSize = Config.Size

        local targetPosition = Config.Position
        local targetSize     = storedTargetSize

        Outer.Visible = true
        Outer.BackgroundTransparency = 0
        Outer.Size = UDim2.fromOffset(targetSize.X.Offset * 0.88, targetSize.Y.Offset * 0.88)
        Outer.Position = UDim2.new(
            targetPosition.X.Scale, targetPosition.X.Offset + (targetSize.X.Offset * 0.06),
            targetPosition.Y.Scale, targetPosition.Y.Offset + (targetSize.Y.Offset * 0.06)
        )
        Outer.BackgroundTransparency = 1

        TweenService:Create(Outer, TweenInfo.new(Library.MenuFadeTime or 0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            Size = targetSize,
            Position = targetPosition,
            BackgroundTransparency = 0,
        }):Play()

        task.wait(Library.MenuFadeTime or 0.2)
        isAnimating = false
    end
    ...
    local function PlayCloseAnimation()
        ...
        local targetSize = UDim2.fromOffset(startSize.X.Offset * 0.88, startSize.Y.Offset * 0.88)
        local targetPos  = UDim2.new(
            startPos.X.Scale, startPos.X.Offset + (startSize.X.Offset * 0.06),
            startPos.Y.Scale, startPos.Y.Offset + (startSize.Y.Offset * 0.06)
        )

        TweenService:Create(Outer, TweenInfo.new(Library.MenuFadeTime or 0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
            Size = targetSize,
            Position = targetPos,
            BackgroundTransparency = 1,
        }):Play()
...
if InputService.TouchEnabled then
    ...
    local BTN_W, BTN_H = 120, 26
    local BTN_GAP      = 34

    -- [MOBILE] Restyled to match the Linoria library look (square, thin border, accent bar)
    local function CreateMobileButton(name, text, startPos)
        ...
        -- subtle top accent bar like the keybind container
    ...
    -- [LOCK] Now actually locks the main window (no drag / no resize)
