...
function Library:UpdateBlur()
    ...
    if Library.BlurEffect then
        if useBlur then
            ...
            local base = Library.BlurSize or 14
            if Library.GlassEnabled then
                base = math.clamp(base, 12, 28)
            end
...
function Library:SetGlass(Enabled, Transparency)
    ...
    -- Glassmorphism stack (CSS-style adapted to Roblox):
    -- 1) BlurEffect = backdrop-filter: blur()
    -- 2) High transparency panels = rgba glass body
    -- 3) Light UIStroke = glass edge border
    -- 4) Top gloss UIGradient = specular highlight
    local L_outer = on and math.clamp(0.82 + (t - 0.5) * 0.25, 0.78, 0.94) or 0
    local L_panel = on and math.clamp(0.55 + (t - 0.5) * 0.5, 0.5, 0.88) or 0
    local L_inner = on and math.clamp(0.35 + (t - 0.5) * 0.4, 0.3, 0.7) or 0
    Library.OuterGlassTransparency = L_outer

    pcall(function()
        if on then
            Library.UseBlur = not (Library.IsMobile == true)
            Library.BlurSize = math.clamp(Library.BlurSize or 14, 8, 28)
            Library.UseDarken = true
            if (Library.DarkenAmount or 0) > 20 then
                Library.DarkenAmount = 12
            end
            Library.DarkenAmount = math.clamp(Library.DarkenAmount or 12, 4, 22)
        end
        ...
        if outer then
            ...
            -- Glass edge (rgba white border)
            local stroke = outer:FindFirstChild("RCLR_GlassEdge")
            if on then
                if not stroke then
                    stroke = Instance.new("UIStroke")
                    stroke.Name = "RCLR_GlassEdge"
            ...
            if on then
                if not shadow then
                    ...
                    shadow.ZIndex = math.max(1, (outer.ZIndex or 1) - 1)
                    shadow.Parent = outer.Parent or outer
                end
                shadow.Size = outer.Size + UDim2.fromOffset(8, 10)
                shadow.Position = outer.Position + UDim2.fromOffset(-4, 4)
                shadow.AnchorPoint = outer.AnchorPoint
                pcall(function()
                    local c = shadow:FindFirstChildOfClass("UICorner")
                    if not c then
            ...
            -- Specular gloss (top highlight gradient)
            ...
            if on then
                if not gloss then
                    ...
                    gloss.ZIndex = (outer.ZIndex or 1) + 1
                    gloss.Parent = outer
        ...
        if Library.DarkOverlay then
            if on and Library.Toggled then
                Library.DarkOverlay.BackgroundTransparency = 1 - ((Library.DarkenAmount or 12) / 100)
            else
                Library.DarkOverlay.BackgroundTransparency = 1
            end
        end
        ...
        for _, d in ipairs(Library.ScreenGui:GetDescendants()) do
            if d:IsA("Frame") or d:IsA("ScrollingFrame") then
                ...
                if nm == "TabArea" or nm == "RCLR_LogoBehind" or nm == "LogoHolder"
                    or nm == "RCLR_GlassGloss" or nm == "RCLR_GlassShadow" or nm == "RCLR_InnerGloss" then
                    if nm ~= "RCLR_GlassGloss" then
                        d.BackgroundTransparency = 1
                    end
                elseif nm == "TabBarOuter" or nm == "TabBarInner" then
                    d.BackgroundColor3 = Library.BackgroundColor
                    d.BackgroundTransparency = on and math.clamp(L_panel * 0.6, 0.25, 0.55) or 0
                    -- thin glass edge on tab bar
