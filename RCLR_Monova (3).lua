...
function Library:CreateWindow(...)
    ...
    function Library:Toggle()
        ...
        if opening then
            Outer.Visible = true;
            Outer.Size = UDim2.fromOffset(math.max(40, fullSize.X.Offset * 0.95), math.max(40, fullSize.Y.Offset * 0.95));
            ...
            local tw = TweenService:Create(Outer, TweenInfo.new(0.15, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
                Size = UDim2.fromOffset(math.max(40, fullSize.X.Offset * 0.94), math.max(40, fullSize.Y.Offset * 0.94));
                BackgroundTransparency = 0.5;
            });
            tw:Play();
...
function Library:SetGlass(Enabled, Transparency)
    ...
    local L_outer = on and math.clamp(t, 0.55, 0.92) or 0
    local L_box = on and math.clamp(t * 0.75, 0.35, 0.78) or 0
    local L_main = on and math.clamp(t * 0.45, 0.2, 0.45) or 0
    ...
    pcall(function()
        ...
        for _, d in ipairs(Library.ScreenGui:GetDescendants()) do
            if d:IsA("Frame") or d:IsA("ScrollingFrame") then
                ...
                -- never break tabs
                if nm == "TabArea" or pname == "TabArea" or nm == "TabBarOuter" or nm == "TabBarInner" then
                    if nm == "TabArea" then
