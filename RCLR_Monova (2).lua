...
function Library:SpawnNotify(Text, Time)
    ...
    -- smooth notify open/close
    Outer.Size = UDim2.fromOffset(8, H);
    ...
    task.spawn(function()
        task.wait(Time or 5);
        local closeTw = TweenService:Create(Outer, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
            Size = UDim2.fromOffset(0, H);
        });
        closeTw:Play();
...
function Library:CreateWindow(...)
    ...
    local function RefreshTabCanvas()
        pcall(function()
            TabArea.CanvasSize = UDim2.fromOffset(math.max(TabListLayout.AbsoluteContentSize.X + 4, 4), 0);
            TabArea.BackgroundTransparency = 1;
        end)
    end
    ...
    do
        ...
        TabArea.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                dragging = true; startX = input.Position.X; startCanvas = TabArea.CanvasPosition.X
            end
        end)
    ...
    function Library:Toggle()
        ...
        if opening then
            Outer.Visible = true;
            Outer.Size = UDim2.fromOffset(math.max(40, fullSize.X.Offset * 0.94), math.max(40, fullSize.Y.Offset * 0.94));
            Outer.BackgroundTransparency = 0.4;
            TweenService:Create(Outer, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
                Size = fullSize;
                BackgroundTransparency = (Library.GlassEnabled and (Library.OuterGlassTransparency or 0.55)) or 0;
            }):Play();
            task.defer(function()
                pcall(function() if Library.ReapplyGlass then Library:ReapplyGlass() end end)
            end)
        else
            local tw = TweenService:Create(Outer, TweenInfo.new(0.16, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
                Size = UDim2.fromOffset(math.max(40, fullSize.X.Offset * 0.92), math.max(40, fullSize.Y.Offset * 0.92));
            ...
            tw.Completed:Connect(function()
                if not Library.Toggled then
                    Outer.Visible = false;
                    Outer.Size = fullSize;
                    Outer.BackgroundTransparency = (Library.GlassEnabled and (Library.OuterGlassTransparency or 0.55)) or 0;
                end
            end)
            pcall(function() if Library._LogoGui then Library._LogoGui.Enabled = false end end)
        end;
...
if InputService.TouchEnabled then
    ...
    local function BindMobileButtonAction(Btn, Outer, ClickAction)
        ...
        InputService.InputChanged:Connect(function(input)
            if input == dragInput and dragging then
                ...
                -- higher threshold so taps still count as clicks on mobile
    ...
    BindMobileButtonAction(LockBtn, LockOuter, function()
        ...
        if Library.Notify then
            pcall(function()
                Library:Notify(IsUnlocked and "UI Unlocked - drag buttons" or "UI Locked")
            end)
        end
    end)
    -- also allow tapping the outer frame of each button
    pcall(function()
        LockOuter.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Touch
                or input.UserInputType == Enum.UserInputType.MouseButton1 then
                -- handled by Btn primarily
            end
        end)
    end)
...
function Library:SetGlass(Enabled, Transparency)
    ...
    if type(Transparency) == "number" then
        Library.GlassTransparency = math.clamp(Transparency, 0.35, 0.95)
    end
    Library.GlassTransparency = Library.GlassTransparency or 0.78
    local on = Library.GlassEnabled == true
    local t = Library.GlassTransparency
    -- BACKGROUND heavily glassed so world shows through
    local L_outer = on and math.clamp(t + 0.05, 0.55, 0.92) or 0
    local L_main  = on and math.clamp(t * 0.55, 0.25, 0.5) or 0
    local L_box   = on and math.clamp(t * 0.7, 0.35, 0.75) or 0
    local L_slide = on and math.clamp(t * 0.4, 0.18, 0.4) or 0
    Library.OuterGlassTransparency = L_outer

    pcall(function()
        if on then
            Library.UseDarken = true
            Library.DarkenAmount = 12
            Library.UseBlur = true
            Library.BlurSize = math.clamp(Library.BlurSize or 10, 4, 18)
        end
        ...
        for _, d in ipairs(Library.ScreenGui:GetDescendants()) do
            if d:IsA("Frame") or d:IsA("ScrollingFrame") then
                ...
                -- TABS stay NORMAL (solid, not black void)
                if nm == "TabArea" then
                ...
                elseif pname == "TabArea" then
                    d.BackgroundTransparency = 0
                elseif nm:find("Fill") or nm:find("Accent") or nm:find("Corner")
                    or nm:find("Cursor") or nm:find("Indicator") or nm:find("Highlight") then
...
function Library:ReapplyGlass()
    Library:SetGlass(Library.GlassEnabled ~= false, Library.GlassTransparency or 0.65)
end
...
function Library:CreateLogo(Info)
    ...
    local speed = tonumber(Info.SpinSpeed) or 45
...
function Library:UpdateLogoVisibility()
    ...
    if Library.Toggled ~= true and Library._LogoGui then
        Library._LogoGui.Enabled = false
    end
end


-- RCLR utility extensions

function Library:Util_1(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
    end
    return r
end

function Library:Util_2(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_10(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_11(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_12(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_13(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_14(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_15(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_16(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_17(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_18(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_19(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_20(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_21(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_22(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_23(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_24(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_25(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_26(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_27(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_28(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_29(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_30(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_31(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_32(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_33(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_34(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_35(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_36(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_37(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_38(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_39(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_40(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_41(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_42(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_43(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_44(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_45(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_46(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_47(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_48(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_49(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_50(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_51(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_52(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_53(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_54(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_55(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_56(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_57(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_58(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_59(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_60(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_61(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_62(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_63(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_64(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_65(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_66(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_67(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_68(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_69(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_70(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_71(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_72(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_73(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_74(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_75(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_76(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_77(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_78(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_79(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_80(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_81(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_82(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_83(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_84(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_85(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_86(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_87(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_88(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_89(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_90(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_91(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_92(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_93(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_94(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_95(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_96(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_97(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_98(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_99(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_100(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_101(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_102(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_103(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_104(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_105(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_106(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_107(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
...
function Library:Util_108(a, b, c)
    a = a or 0; b = b or 0; c = c or 0
    local r = (tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.3 + (tonumber(c) or 0) * 0.2
    if Library and Library.MapValue then
        r = Library:MapValue(math.clamp(r, 0, 100), 0, 100, 0, 1)
