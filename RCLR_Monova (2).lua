...
function Library:CreateWindow(...)
    ...
    -- middle mouse wheel + drag (mobile) — only scrolls after move so tab clicks still work
    do
        ...
        InputService.InputChanged:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseWheel then
                TabArea.CanvasPosition = Vector2.new(
                    math.max(0, TabArea.CanvasPosition.X - input.Position.Z * 48),
                    0
                )
                return
            end
    ...
    pcall(function() if Library.SetGlass then Library:SetGlass(true, Library.GlassTransparency or 0.88) end end)
    ...
    task.defer(function()
        pcall(function()
            ...
            if not Library._LogoGui and Library.CreateLogo then
...
function Library:SetGlass(Enabled, Transparency)
    ...
    if type(Transparency) == "number" then
        Library.GlassTransparency = math.clamp(Transparency, 0.4, 0.95)
    end
    Library.GlassTransparency = Library.GlassTransparency or 0.82
    Library._GlassDirty = true

    -- throttle: max ~8 applies/sec so dragging glass slider doesn't lag
    local now = os.clock()
    if Library._LastGlassApply and (now - Library._LastGlassApply) < 0.12 then
    ...
    local L_outer = on and math.clamp(t, 0.7, 0.95) or 0
    local L_box   = on and math.clamp(t * 0.7, 0.35, 0.7) or 0
    local L_main  = on and math.clamp(t * 0.4, 0.15, 0.4) or 0
    ...
    pcall(function()
        if on then
            if Library.IsMobile == true then
                ...
                Library.BlurSize = math.clamp(Library.BlurSize or 8, 2, 14)
            end
        end
...
function Library:ReapplyGlass()
    Library:SetGlass(Library.GlassEnabled ~= false, Library.GlassTransparency or 0.88)
end
...
function Library:CreateLogo(Info)
    ...
    local size = tonumber(Info.TextSize) or 48
    local speed = tonumber(Info.SpinSpeed) or 35
    ...
    -- two logos on the SIDES so center title stays readable
    local function makeSide(anchorX, posX)
        local holder = Instance.new("Frame")
        holder.BackgroundTransparency = 1
        holder.AnchorPoint = Vector2.new(anchorX, 0.5)
        holder.Position = UDim2.new(posX, 0, 0.5, 0)
        holder.Size = UDim2.fromOffset(size * 2.2, size * 2.2)
        holder.Parent = gui
        ...
        label.TextSize = size
        label.TextColor3 = Library.AccentColor
        label.TextTransparency = 0.72
        label.Parent = holder
        return holder, label
    end
    local leftH, leftL = makeSide(0, 0.08)
    local rightH, rightL = makeSide(1, 0.92)
    local angle = 0
    local acc = 0
    local step = (Library.IsMobile == true) and (1/20) or (1/30)
    local conn = RunService.Heartbeat:Connect(function(dt)
        ...
        leftH.Rotation = angle
        rightH.Rotation = angle
        leftL.TextColor3 = Library.AccentColor
        rightL.TextColor3 = Library.AccentColor
    end)
...
function Library:AddNotifyOptions(Groupbox)
    Groupbox:AddSlider('NotifyTransparency', {
        ...
        Default = Library.NotifyConfig.Transparency or 60;
    ...
    Groupbox:AddSlider('NotifyPosX', {
        Text = 'Notify Pos X';
        Default = Library.NotifyConfig.PosX or 50;
        Min = 0; Max = 100; Rounding = 0; Suffix = '%';
        Callback = function(V)
    ...
    Groupbox:AddSlider('NotifyPosY', {
        Text = 'Notify Pos Y';
        Default = Library.NotifyConfig.PosY or 60;
        Min = 0; Max = 100; Rounding = 0; Suffix = '%';
        Callback = function(V)
    ...
    Groupbox:AddButton('Test Notify Style', function()
