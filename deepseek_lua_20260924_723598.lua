--[[
  RCLR Monova - FIXED IsMobile (2026-09-24) + Restored Monova-5 Glass
  If you still see "attempt to index nil with IsMobile", GitHub is caching the OLD file.
  Force refresh: add ?v=3 to the URL or rename the file.
]]
...
table.insert(Library.Signals, RunService.Heartbeat:Connect(function(Delta)
    -- throttled rainbow (cheaper than RenderStepped every frame)
...
-- PC / Mobile performance profile (AFTER Library exists)
do
    local mobile = false
    pcall(function()
...
function Library:UpdateColorsUsingRegistry()
    if Library._UpdatingColors then return end
    Library._UpdatingColors = true
    ...
    Library._UpdatingColors = false
    -- do NOT call ReapplyGlass here (freeze loop)
end
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        function Slider:Display()
            ...
            if Slider._FillTween then
                pcall(function() Slider._FillTween:Cancel() end)
            end
...
function Library:CreateWindow(...)
    ...
    do
        local dragging, startX, startCanvas = false, 0, 0
        TabArea.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1
                ...
                startCanvas = TabArea.CanvasPosition.X
            end
        end)
        InputService.InputChanged:Connect(function(input)
            ...
            if input.UserInputType == Enum.UserInputType.MouseMovement
                or input.UserInputType == Enum.UserInputType.Touch then
                local dx = startX - input.Position.X
                if math.abs(dx) > 8 then
                    TabArea.CanvasPosition = Vector2.new(math.max(0, startCanvas + dx), 0)
                end
            end
        end)
    ...
    function Window:AddTab(Name)
        ...
        function Tab:ShowTab()
            ...
            task.defer(function() pcall(function() if Library.ReapplyGlass then Library:ReapplyGlass() end end) end)
        end;
...
if InputService.TouchEnabled then
    ...
    Library.UpdateColorsUsingRegistry = function(self) _origUpdate(self) end
end

-- =========================================================
-- GLASS FUNCTIONS (RESTORED FROM MONOVA 5)
-- =========================================================
...
-- [FIXED GLASS — RESTORED MONOVA-5 VERSION] Full glass everywhere, no washed-out look
function Library:SetGlass(Enabled, Transparency)
    ...
    -- FULL glass everywhere (not only top bar)
    ...
    pcall(function()
        ...
        for _, d in ipairs(Library.ScreenGui:GetDescendants()) do
            if d:IsA("Frame") or d:IsA("ScrollingFrame") then
                ...
                if nm == "TabArea" or nm == "RCLR_LogoBehind" or nm == "LogoHolder" then
                ...
                if on and d.BorderSizePixel and d.BorderSizePixel > 0 then
                    pcall(function()
                        d.BorderColor3 = Library.OutlineColor
                    end)
                end
...
function Library:SyncThemeColorPickers()
    if Library._SyncingPickers then return end
    Library._SyncingPickers = true
    pcall(function()
        local function setCP(idx, color)
            local opt = Options and Options[idx]
            if not opt or not color then return end
            opt.Value = color
            pcall(function()
                if opt.SetHSVFromRGB then opt:SetHSVFromRGB(color) end
            end)
            pcall(function()
                if opt.Display then opt:Display() end
            end)
        end
        setCP("CP_Accent", Library.AccentColor)
        setCP("CP_BG", Library.BackgroundColor)
        setCP("CP_Main", Library.MainColor)
        setCP("CP_Outline", Library.OutlineColor)
        setCP("CP_Font", Library.FontColor)
    end)
    Library._SyncingPickers = false
end
...
function Library:AddNotifyOptions(Groupbox)
    ...
    Groupbox:AddSlider('NotifyTransparency', {
        ...
        Callback = function(V)
            Library.NotifyConfig.Transparency = V
        end;
    })
    ...
    Groupbox:AddSlider('NotifyMaxHeight', {
        ...
        Callback = function(V)
            Library.NotifyConfig.MaxHeight = V
        end;
    })
    Groupbox:AddDropdown('NotifyBarSide', {
        ...
        Default = 1;
        Callback = function(V)
            Library.NotifyConfig.BarSide = V
        end;
    })
    Groupbox:AddDropdown('NotifyAlign', {
        ...
        Default = 1;
        Callback = function(V)
            Library.NotifyConfig.Alignment = V
        end;
    })
    ...
    Groupbox:AddToggle('NotifyClip', {
        ...
        Callback = function(V)
            Library.NotifyConfig.ClipDescendants = V
        end;
    })
...
-- RCLR performance helpers (kept from Monova 8; harmless)
function Library:_PerfHelper_1(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_2(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_3(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_4(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_5(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_6(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_7(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_8(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_9(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_10(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_11(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_12(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_13(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_14(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_15(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_16(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_17(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_18(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_19(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_20(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_21(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_22(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_23(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_24(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_25(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_26(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_27(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_28(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_29(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_30(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_31(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_32(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_33(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_34(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_35(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_36(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_37(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_38(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_39(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_40(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_41(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_42(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_43(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_44(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_45(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_46(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_47(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_48(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_49(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_50(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_51(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_52(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
function Library:_PerfHelper_53(a, b) if Library.IsMobile == true then return ((tonumber(a) or 0) * 0.5 + (tonumber(b) or 0) * 0.5) * 0.85 end return (tonumber(a) or 0) * 0.6 + (tonumber(b) or 0) * 0.4 end
