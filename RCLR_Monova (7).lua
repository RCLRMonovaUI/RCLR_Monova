--[[
  RCLR Monova - FIXED IsMobile (2026-09-24)
  If you still see "attempt to index nil with IsMobile", GitHub is caching the OLD file.
  Force refresh: add ?v=2 to the URL or rename the file.
]]
...
do
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        -- [FIXED] Cancel previous tween, recalc size, smoother easing
        ...
        SliderInner.InputBegan:Connect(function(Input)
            if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and not Library:MouseIsOverOpenedFrame() then
                local function UpdateSlider(PosX)
                    -- use track width so drag works after resize
...
function Library:CreateWindow(...)
    ...
    -- Tabs: DRAG only (no mouse wheel). Wheel is left for option lists only.
    pcall(function()
        TabArea.ScrollingEnabled = false -- blocks wheel from shifting tabs
    end)
    do
        ...
        InputService.InputChanged:Connect(function(input)
            ...
            -- NEVER touch tabs on MouseWheel
    ...
    -- [RESIZE HANDLER – copied from source]
    ...
    function Window:AddTab(Name)
        ...
        function Tab:ShowTab()
            ...
            -- [GLASS] Reapply glass on tab switch
    ...
    -- [ANIM] Toggle with proper open & close animations
    ...
    function Library:Toggle()
        ...
        if Library.Toggled then
            ...
            task.spawn(function()
                ...
                -- skip custom cursor on mobile (saves FPS)
                if Library.IsMobile == true then
                    ...
                    while Library.Toggled and ScreenGui.Parent do
                        ...
                        if Library.PerformanceMode then
                            task.wait() -- yield 1 frame, cheaper than exclusive RenderStepped bind
                        else
                            RenderStepped:Wait();
                        end
                    end
...
if InputService.TouchEnabled then
    ...
    -- backup: Activated on TextButtons (more reliable on some executors)
    pcall(function()
...
-- [FIXED GLASS] More transparent, covers every registered frame
function Library:SetGlass(Enabled, Transparency)
    if Library._SettingGlass then return end
    Library._SettingGlass = true
    local function _finish() Library._SettingGlass = false end
    ...
    if type(Transparency) == "number" then
        Library.GlassTransparency = math.clamp(Transparency, 0.5, 0.97)
    end
    Library.GlassTransparency = Library.GlassTransparency or 0.88
    ...
    -- cooler glass: clear content, solid top/tabs so not washed white
    local L_outer = on and math.clamp(t * 0.92, 0.75, 0.94) or 0
    local L_content = on and math.clamp(t * 0.85, 0.55, 0.88) or 0
    local L_main = on and math.clamp(t * 0.55, 0.35, 0.65) or 0
    Library.OuterGlassTransparency = L_outer

    pcall(function()
        if on then
            Library.UseDarken = true
            -- never let darken slam to 100 and bleach the UI
            if (Library.DarkenAmount or 0) > 25 then
                Library.DarkenAmount = 8
            else
                Library.DarkenAmount = math.clamp(Library.DarkenAmount or 8, 0, 25)
            end
            if Library.IsMobile == true then
                Library.UseBlur = false
                Library.BlurSize = 0
                ...
                Library.BlurSize = math.clamp(Library.BlurSize or 5, 2, 10)
            end
        end
        ...
        if Library.DarkOverlay then
            if on and Library.Toggled then
                Library.DarkOverlay.BackgroundTransparency = 1 - (math.clamp(Library.DarkenAmount or 8, 0, 25) / 100)
            else
                Library.DarkOverlay.BackgroundTransparency = 1
            end
        end
        ...
        for _, d in ipairs(Library.ScreenGui:GetDescendants()) do
            if d:IsA("Frame") or d:IsA("ScrollingFrame") then
                ...
                elseif nm == "TabBarOuter" or nm == "TabBarInner" then
                    -- tabs strip: keep darker / readable, NOT full glass
                    d.BackgroundColor3 = Library.BackgroundColor
                    d.BackgroundTransparency = on and 0.15 or 0
                    pcall(function()
                ...
                elseif nm == "Outer" or d == Library.WindowOuter then
                    ...
                    if bg == "BackgroundColor" then
                        d.BackgroundColor3 = Library.BackgroundColor
                        d.BackgroundTransparency = L_content
            ...
            elseif on and d:IsA("UIStroke") then
                pcall(function()
                    if d.Name == "RCLR_GlassStroke" then
                        d.Color = Library.OutlineColor
                        d.Transparency = 0.45
                    elseif d.Parent and (d.Parent.Name == "TabBarOuter" or d.Parent.Name == "TabBarInner" or (d.Parent.Parent and d.Parent.Parent.Name == "TabArea")) then
                        -- keep tab lines darker
                        d.Transparency = 0.1
                        d.Color = Library.OutlineColor
                    else
                        d.Transparency = math.clamp((d.Transparency or 0) + 0.2, 0.15, 0.7)
                    end
                end)
            end
        end
        ...
        -- resize corner matches accent
        pcall(function()
            local outer = Library.WindowOuter
            if not outer then return end
            for _, c in ipairs(outer:GetChildren()) do
                if c:IsA("ImageLabel") or c:IsA("Frame") then
                    if c.Name:find("Corner") or c.Size.X.Offset == 42 or c.Size.X.Offset == 46 then
                        c.BackgroundColor3 = Library.AccentColor
                    end
                end
            end
        end)
...
function Library:CreateLogo(Info)
    ...
    if not outer or not outer.Parent then
        -- retry shortly once window exists
        task.defer(function()
            task.wait(0.15)
    ...
    -- Logo lives INSIDE the window, ZIndex BELOW content so it is behind the UI
    ...
    holder.ZIndex = 1  -- Outer is 1; content is higher
    ...
    Library._LogoGui = holder  -- so Unload paths that destroy _LogoGui still work
    ...
    local conn = RunService.Heartbeat:Connect(function(dt)
        ...
        -- accent color only every few ticks
...
-- RCLR performance / glass / notify helpers

function Library:_PerfHelper_1(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
    end
    return a * 0.6 + b * 0.4
end

function Library:_PerfHelper_2(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_3(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_4(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_5(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_6(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_7(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_8(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_9(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_10(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_11(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_12(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_13(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_14(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_15(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_16(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_17(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_18(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_19(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_20(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_21(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_22(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_23(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_24(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_25(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_26(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_27(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_28(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_29(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_30(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_31(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_32(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_33(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_34(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_35(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_36(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_37(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_38(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_39(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_40(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_41(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_42(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_43(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_44(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_45(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_46(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_47(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_48(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_49(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_50(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_51(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_52(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
...
function Library:_PerfHelper_53(a, b)
    a = tonumber(a) or 0
    b = tonumber(b) or 0
    if Library.IsMobile == true then
        return (a * 0.5 + b * 0.5) * 0.85
